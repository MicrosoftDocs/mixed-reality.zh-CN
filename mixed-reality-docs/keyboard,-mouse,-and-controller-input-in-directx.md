...
最后, 更新输入逻辑以识别控制器状态的更改。 此处, 我们使用上述部分中讨论的相同 m_pointerPressed 变量来添加鼠标事件。 将此添加到 Update 方法, 就像它在何处检查 SpatialPointerPose:
...
* 设计应用程序时, 不要依赖 (例如) 游戏的 WASD 和鼠标外观控件。 HoloLens 旨在使用户能够浏览聊天室。 在这种情况下, 用户直接控制相机。 使用移动/外观控制来围绕房间来驱动照相机的接口不能提供相同的体验。
