...
直接操作是一个输入模型，涉及到用手直接触摸全息影像。 直接操作的理念是使对象的行为如同在现实世界中一样自然。 只需按下按钮就能将其激活，抓取对象可以拾取对象，2D 内容的行为与在虚拟触摸屏上类似。 因此，直接操作很容易学会，而且充满了乐趣。 直接操作被视为一种“近距”输入模型，这意味着，它最好用于与手臂可触及范围内的内容进行交互。
...
如果不使用随机的一般形状，我们建议使用球体碰撞体。 然后，以可视方式渲染它，以便为近距定位提供更好的线索。 该球体的直径应与食指的粗细相匹配，以提高触摸准确度。 可以通过调用手部 API 来更轻松地检索手指粗细的变量。
...
* 第二个机制是沉降。 在指尖接触到按钮后，沉降可产生一种按入感。 该机制的工作原理是，按钮在深度轴上如影随形地与指尖一起移动。 当指尖达到指定的深度（按下按钮），或者在移过按钮后脱离该深度（松开按钮）时，可以触发按钮。
* 触发按钮时，应该添加音效来增强反馈。
...
* 抓取操作视觉元素，通过边角视觉元素执行统一的缩放，然后通过边缘视觉元素进行重排。
...
使用基于视觉元素的操作可以通过边界框及其周围的操作视觉元素来操作 3D 对象。 一旦用户的手靠近某个 3D 对象，就会显示边界框和最靠近的视觉元素。 用户可以抓取边界框来移动整个对象，抓取边缘视觉元素来旋转对象，或者抓取边角视觉元素进行统一的缩放。
...
如果直接操作按预期方式运行，则用户可能会觉得它充满魔力。 但是，如果只能在无意触发全息影像的情况下才能四处移动手部，则直接操作可能很快就会带来困扰。 眼动追踪可能有助于更好地识别用户的意图。
...
* **何处**：使用有关用户正在使用快速投射手势观看的内容的信息。 抓取一幅全息影像，然后大致将它投射到预期目标。  
    ...
    尽管有时可以正常进行此操作，但快速执行手势可能会导致目标极不准确。 在此情况下，包含眼动跟踪可以提高手势的准确度。
