...
# <a name="mr-input-211-gesture"></a>MR 输入211：手势

[手势](gaze-and-commit.md#composite-gestures)会使用户意向变为操作。 借助笔势，用户可以与全息影像交互。 在本课程中，我们将了解如何跟踪用户的动手，响应用户输入，并根据手头状态和位置向用户提供反馈。

>[!VIDEO https://www.youtube.com/embed/c9zlpfFeEtc]

在[先生/女士 101](holograms-101.md)中，我们使用了一种简单的点击手势来与全息影像交互。 现在，我们将转到轻攻攻攻，并探索新的概念：
...
<td>MR 输入211：手势</td><td style="text-align: center;"> ✔️</td><td style="text-align: center;"> ✔️</td>
</tr>
</table>
...
3. 在 "**层次结构**" 面板中单击 " **InputManager** "，然后将**CursorWithFeedback**对象从**层次结构**拖到**检查器**。
...
3. 将手置于 "就绪" 位置（食指指向天空），以便 HoloLens 开始跟踪。
...
* 开始旋转 astronaut，使其具有导航手势（将您的索引手指同时一起）。
* 向左、向右、向上、向下移动。
...
10. 单击 "响应" 下的 **+** ，并将 " **AstronautManager** " 从**层次结构**拖到 "**无（对象）** " 字段。
11. 现在，单击 "**无函数**" 下拉列表，选择 " **AstronautManager**"，然后选择 " **ExpandModelCommand**"。
12. 单击语音输入处理程序的 **+** "按钮，然后从" 关键字 "下拉列表中选择"**重置模型**"。
13. 单击 "响应" 下的 **+** ，并将 " **AstronautManager** " 从**层次结构**拖到 "**无（对象）** " 字段。
...
恭喜你！ 你现在已经完成了**MR 输入211：手势**。

* 你知道如何检测和响应手动跟踪、导航和操作事件。
* 了解导航和操作笔势之间的差异。
* 你知道如何更改光标以提供有关何时检测到手、即将丢失以及何时对象支持不同交互（导航与操作）的视觉反馈。
