# <a name="mixing-color-and-depth-data"></a>混合使用颜色和深度数据

从颜色照相机和深度照相机中获取的资源都具有不同的形状和大小 ！ 如果你想要合并两个，没有执行大量操作可以执行以将它们相互对齐。
...
[获取数据](#Getting-Data)  
[对数据进行转换](#Transforming-Data)  
[从点云中创建网格](#Creating-a-Mesh-From-the-Point-Cloud)  
[从深度和颜色创建映像](#Creating-an-Image-From-Depth-and-Color)  
[清理](#Cleaning-Up)  
[完整源](#Full-Source)  
...
虽然我们可以在此处使用几乎任何我们想要的配置，我们要坚持较小的解决方法，因为我们将向文件写入未压缩的数据。 检查上从读取文档[颜色照相机]()并[深度照相机]()有关配置详细信息 ！

一个额外的元素是`synchronized_images_only`选项 ！ 将其设置为`true`指定我们只需要具有这两种颜色的捕获<i>和</i>填充的深度。 如果我们不使用此，从捕获的第几个`k4a_device_get_capture`可能缺少任一深度或彩色图像。
...
现在，我们有我们的配置信息，我们需要设置将传递到所有我们转换函数的校准对象 ！ 此对象包含预先计算用于转换与颜色深度空间和 visa 迁移照相机空间坐标的资源。 使其中一个是非常简单：
...
现在，我们需要获取彩色图像，并深图像 ！ 因此，在这里捕获单个帧和从它的相关数据。 如果你的映像之一传入回 null，则确保您使用`synchronized_images_only`在配置中 ！
...
## <a name="transforming-data"></a>对数据进行转换

`k4a_transform_`函数将转换到另一个图像数据从一个照相机空间 ！ 生成的映像将匹配的失真和解决方法的目标照相机空间，因此关联颜色和深度信息一起轻松地。

例如 ！ 很多情况下您可能希望从照相机，每个颜色的像素将相应深度的彩色图像 ！ 为此，会将深度数据转换为颜色照相机空间。

或者，你可能想要从深度相机，创建三维点数据的云并知道每个点是何种颜色。 为此，我们可以将深度数据转换为一个点云和，并将彩色图像转换为深度照相机空间 ！ 然后将具有 XYZ 值的缓冲区和相应的缓冲区的颜色。

### <a name="depth-data-in-color-space"></a>颜色空间中的深度数据

这将转换深度数据，以匹配颜色照相机的输出 ！ 得到的缓冲区将具有彩色图像中每个像素的 16 位深度值。
...
### <a name="creating-a-point-cloud"></a>创建一个点云和

点云也存储在图像资源 ！ 在这里，我们创建格式的映像`K4A_IMAGE_FORMAT_CUSTOM`，因为一个点云和不完全是一个图像。 在图中每个像素将组成 3`int16_t`表示 x y Z 坐标的值，因此我们还正确的大小为指定的。
...
### <a name="color-data-in-depth-space"></a>颜色深度空间中的数据

这将转换颜色数据，以匹配深度照相机的输出 ！ 得到的缓冲区中每个颜色值将对应的深度图像中的值。
...
## <a name="creating-a-mesh-from-the-point-cloud"></a>从点云中创建网格

![](img/ColorandDepthMeshSmall.png)

我们首先将点云数据转换为更熟悉的格式。 我们将从毫米为单位转换为指标，并将它移动到浮点数。
...
现在我们将拼结向上这些顶点的人脸的一些信息 ！ 由于所有内容的布局方式为网格中，这是一个简单的过程的链接上每个单元格的角。 我们要做的唯一附加件事就跳过任何包含空数据的人脸。 如果 Kinect 找不到深度数据的中点云的点，它将其放在 < 0,0,0 > ！ 它将转换非常 hedgehog，如果您将它们添加到索引列表。
...
然后我们可以只是此信息写入文件 ！ 我们不会删除未使用的颜色，或由于位置中创建索引，因此我们可以只需传递到目前为止我们已创建的缓冲区中增加了复杂性。
...
## <a name="creating-an-image-from-depth-and-color"></a>从深度和颜色创建映像

![](img/ColorandDepthSmall.png)

使用颜色，这就很容易做到 ！ 在这里，我们将执行简单淡化为黑色的距离，但无法执行各种各样的有趣的事情，如剪切掉某些深度范围，或在用作照片操作工具中的掩码的 alpha 通道中存储深度 ！
...
## <a name="next-lab---accessing-the-imuusingimumd"></a>下一步实验室-[访问 IMU](UsingIMU.md)