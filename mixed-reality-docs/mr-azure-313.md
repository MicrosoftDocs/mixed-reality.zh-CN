...
<span data-ttu-id="a1f1b-149">以下硬件和软件是必需的：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-149">The following hardware and software is required:</span></span>
...
<span data-ttu-id="a1f1b-166">有关校准的帮助，请遵循此[HoloLens 校准文章链接](calibration.md#hololens)。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-166">For help on Calibration, please follow this [link to the HoloLens Calibration article](calibration.md#hololens).</span></span>
...
<span data-ttu-id="a1f1b-181">但是，如果你想要使用你自己的自定义影像模型可以执行以下步骤：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-181">However, you can follow these steps if you wish to use your own Custom Vision model:</span></span>
...
2. <span data-ttu-id="a1f1b-199">转到**创建资源**并查找**容器注册表**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-199">Go to **Create a resource** and look for **Container Registry**.</span></span>
...
5.  <span data-ttu-id="a1f1b-232">在页面底部单击**下一步：大小和小数位数**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-232">On the bottom of the page click on **Next: Size and scale**.</span></span>
...
14. <span data-ttu-id="a1f1b-252">上*设备详细信息*页上显示，需要一份**连接字符串**（主键）。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-252">On the *Device Details* page that appears, take a copy of the **Connection String** (primary key).</span></span>
...
3.  <span data-ttu-id="a1f1b-266">[Visual Studio Code (也称为 VS Code)](https://code.visualstudio.com/download)。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-266">[Visual Studio Code (also known as VS Code)](https://code.visualstudio.com/download).</span></span>
...
1. <span data-ttu-id="a1f1b-303">打开**VS 代码**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-303">Open **VS Code**.</span></span>
...
7. <span data-ttu-id="a1f1b-322">一次**Cookiecutter**已完成安装，应重新启动你的计算机，以便**Cookiecutter**被识别为您的系统的环境中的命令。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-322">Once **Cookiecutter** has finished installing, you should restart your machine, so that **Cookiecutter** is recognized as a command, within your System's environment.</span></span>
...
8. <span data-ttu-id="a1f1b-341">删除**localhost:5000**，并在其位置插入\*容器注册表\***登录服务器**地址，创建时记下**容器注册表服务**([在步骤 8 的第 2 章](#chapter-2---the-container-registry-service))。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-341">Delete **localhost:5000**, and in its place insert the *Container Registry* **Login Server** address, which you noted when creating the **Container Registry Service** ([in step 8, of Chapter 2](#chapter-2---the-container-registry-service)).</span></span> <span data-ttu-id="a1f1b-342">按**Enter**密钥，以确认该地址。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-342">Press the **Enter** key, to confirm the address.</span></span>
...
9. <span data-ttu-id="a1f1b-405">若要查看从你的设备发送到 IoT 中心的消息，再次右键单击设备名称在**Azure IoT 中心设备**部分中，在**资源管理器**面板，并单击**启动监视D2C 消息**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-405">To view the messages sent from your device to the IoT Hub, right-click again on your device name in the **Azure IoT Hub Devices** section, in the **Explorer** panel, and click on **Start Monitoring D2C Message**.</span></span> <span data-ttu-id="a1f1b-406">从你的设备发送的消息应显示在 VS 终端。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-406">The messages sent from your device should appear in the VS Terminal.</span></span> <span data-ttu-id="a1f1b-407">耐心等待，因为这可能需要一些时间。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-407">Be patient, as this may take some time.</span></span> <span data-ttu-id="a1f1b-408">请参阅下一章中的进行调试和检查部署是否成功。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-408">See the next Chapter for debugging, and checking if deployment was successful.</span></span>

<span data-ttu-id="a1f1b-409">现在，此模块将循环访问中的映像之间**映像**文件夹和分析，每次迭代。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-409">This module will now iterate between the images in the **images** folder and analyze them, with each iteration.</span></span> <span data-ttu-id="a1f1b-410">这是很明显只是演示如何获取要在 IoT Edge 设备环境中工作的基本机器学习模型。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-410">This is obviously just a demonstration of how to get the basic machine learning model to work in an IoT Edge device environment.</span></span> 

<span data-ttu-id="a1f1b-411">若要扩展此示例中的功能，可以继续以下几种方式。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-411">To expand the functionality of this example, you could proceed in several ways.</span></span> <span data-ttu-id="a1f1b-412">一种方法可能包括一些代码中的容器，用于捕获从网络摄像头，连接到设备，然后将映像保存在 images 文件夹中的照片。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-412">One way could be including some code in the container, that captures photos from a webcam that is connected to the device, and saves the images in the images folder.</span></span> 

<span data-ttu-id="a1f1b-413">另一种方法可以将复制映像 IoT 设备到容器。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-413">Another way could be copying the images from the IoT device into the container.</span></span> <span data-ttu-id="a1f1b-414">一个实用方法可执行此操作是在 IoT 设备 （可能是一个小型应用无法执行该作业，如果想要自动执行此过程） 的终端中运行以下命令。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-414">A practical way to do that is to run the following command in the IoT device Terminal (perhaps a small app could do the job, if you wished to automate the process).</span></span> <span data-ttu-id="a1f1b-415">可以通过从存储文件的文件夹位置手动运行它来测试此命令：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-415">You can test this command by running it manually from the folder location where your files are stored:</span></span>

```bash
    sudo docker cp <filename> <modulename>:/app/images/<a name of your choice>
```

## <a name="chapter-10---debugging-the-iot-edge-runtime"></a><span data-ttu-id="a1f1b-416">第 10 章 — 调试 IoT Edge 运行时</span><span class="sxs-lookup"><span data-stu-id="a1f1b-416">Chapter 10 - Debugging the IoT Edge Runtime</span></span>

<span data-ttu-id="a1f1b-417">以下是命令行和技巧，以帮助您监视和调试的消息传递活动的列表*IoT Edge 运行时*，从你**Ubuntu 设备**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-417">The following are a list of command lines, and tips, to help you monitor and debug the messaging activity of the *IoT Edge Runtime*, from your **Ubuntu device**.</span></span> 

- <span data-ttu-id="a1f1b-418">检查*IoT Edge 运行时*通过运行以下命令行的状态：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-418">Check the *IoT Edge Runtime* status by running the following command line:</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="a1f1b-419">请记住，按**Ctrl + C**以完成查看状态。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-419">Remember to press **Ctrl + C**, to finish viewing the status.</span></span>

- <span data-ttu-id="a1f1b-420">列出当前已部署的容器。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-420">List the containers that are currently deployed.</span></span> <span data-ttu-id="a1f1b-421">如果*IoT 中心服务*是否容器成功部署，它们将显示通过运行以下命令行：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-421">If the *IoT Hub Service* has deployed the containers successfully, they will be displayed by running the following command line:</span></span>

    ```bash
        sudo iotedge list
    ```

    <span data-ttu-id="a1f1b-422">或</span><span class="sxs-lookup"><span data-stu-id="a1f1b-422">Or</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="a1f1b-423">以上是一种检查是否在模块是否已成功部署，因为它将出现在列表中; 很好方法否则将**仅**请参阅*edgeHub*并*edgeAgent*。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-423">The above is a good way to check whether your module has been deployed successfully, as it will appear in the list; you will otherwise **only** see the *edgeHub* and *edgeAgent*.</span></span>

- <span data-ttu-id="a1f1b-424">若要显示容器的代码日志，请运行以下命令行：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-424">To display the code logs of a container, run the following command line:</span></span>

    ```bash
        journalctl -u iotedge
    ```

<span data-ttu-id="a1f1b-425">**用于管理 IoT Edge 运行时的有用命令：**</span><span class="sxs-lookup"><span data-stu-id="a1f1b-425">**Useful commands to manage the IoT Edge Runtime:**</span></span>

-  <span data-ttu-id="a1f1b-426">若要删除所有容器主机中：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-426">To delete all containers in the host:</span></span>

    ```bash
        sudo docker rm -f $(sudo docker ps -aq)
    ```

-  <span data-ttu-id="a1f1b-427">若要停止*IoT Edge 运行时*:</span><span class="sxs-lookup"><span data-stu-id="a1f1b-427">To stop the *IoT Edge Runtime*:</span></span>

    ```bash
        sudo systemctl stop iotedge
    ```

## <a name="chapter-11---create-table-service"></a><span data-ttu-id="a1f1b-428">章 11-创建表服务</span><span class="sxs-lookup"><span data-stu-id="a1f1b-428">Chapter 11 - Create Table Service</span></span> 

<span data-ttu-id="a1f1b-429">导航回到你的 Azure 门户，其中将创建 Azure 表服务，通过创建存储资源。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-429">Navigate back to your Azure Portal, where you will create an Azure Tables Service, by creating a Storage resource.</span></span>

1. <span data-ttu-id="a1f1b-430">如果尚未登录，登录到[Azure 门户](https://portal.azure.com)。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-430">If not already signed in, log into the [Azure Portal](https://portal.azure.com).</span></span>

2. <span data-ttu-id="a1f1b-431">登录后，单击**创建资源**，在左上角，并搜索**存储帐户**，然后按**Enter**键，若要开始搜索。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-431">Once logged in, click on **Create a resource**, in the top left corner, and search for **Storage account**, and press the **Enter** key, to start the search.</span></span>

3. <span data-ttu-id="a1f1b-432">一旦它已出现，请单击**存储帐户-blob、 文件、 表、 队列**从列表中。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-432">Once it has appeared, click **Storage account - blob, file, table, queue** from the list.</span></span>

    ![搜索存储帐户](images/AzureLabs-Lab313-35.png)

4. <span data-ttu-id="a1f1b-434">该新页面将提供的说明**存储帐户**服务。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-434">The new page will provide a description of the **Storage account** Service.</span></span> <span data-ttu-id="a1f1b-435">在左下角此提示，请单击**创建**按钮，以创建服务的此实例。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-435">At the bottom left of this prompt, click the **Create** button, to create an instance of this Service.</span></span>

    ![创建存储实例](images/AzureLabs-Lab313-36.png)

5. <span data-ttu-id="a1f1b-437">一旦你单击**创建**，将显示一个面板：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-437">Once you have clicked on **Create**, a panel will appear:</span></span>

    1. <span data-ttu-id="a1f1b-438">插入所需**名称**此服务实例 (*必须全部小写*)。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-438">Insert your desired **Name** for this Service instance (*must be all lowercase*).</span></span>

    2. <span data-ttu-id="a1f1b-439">有关**部署模型**，单击**资源管理器**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-439">For **Deployment model**, click **Resource manager**.</span></span>

    3. <span data-ttu-id="a1f1b-440">有关**帐户类型**，使用下拉列表菜单，单击**存储 (常规用途 v1)**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-440">For **Account kind**, using the dropdown menu, click **Storage (general purpose v1)**.</span></span>

    4. <span data-ttu-id="a1f1b-441">单击相应**位置**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-441">Click an appropriate **Location**.</span></span>
    
    5. <span data-ttu-id="a1f1b-442">有关**复制**下拉菜单中，单击**读取-访问-异地冗余存储 (RA-GRS)**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-442">For the **Replication** dropdown menu, click **Read-access-geo-redundant storage (RA-GRS)**.</span></span>

    6. <span data-ttu-id="a1f1b-443">有关**性能**，单击**标准**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-443">For **Performance**, click **Standard**.</span></span>

    7. <span data-ttu-id="a1f1b-444">内**需要安全传输**部分中，单击**禁用**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-444">Within the **Secure transfer required** section, click **Disabled**.</span></span>

    8. <span data-ttu-id="a1f1b-445">从**订阅**下拉菜单中，单击相应的订阅。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-445">From the **Subscription** dropdown menu, click an appropriate subscription.</span></span>

    9. <span data-ttu-id="a1f1b-446">选择**资源组**或新建一个。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-446">Choose a **Resource Group** or create a new one.</span></span> <span data-ttu-id="a1f1b-447">资源组提供了一种方法来监视、 控制访问，预配，以及管理 Azure 资产的集合计费。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-447">A resource group provides a way to monitor, control access, provision, and manage, billing for a collection of Azure assets.</span></span> <span data-ttu-id="a1f1b-448">建议将所有 Azure 服务与常见的资源组下的单个项目 （例如如这些课程））。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-448">It is recommended to keep all the Azure Services associated with a single project (e.g. such as these courses) under a common resource group).</span></span>

        > <span data-ttu-id="a1f1b-449">如果你想要阅读更多有关 Azure 资源组，请遵循此[如何管理资源组链接](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-portal)。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-449">If you wish to read more about Azure Resource Groups, please follow this [link on how to manage a Resource Group](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-portal).</span></span>

    10. <span data-ttu-id="a1f1b-450">将保留**虚拟网络**作为**禁用**，如果这是为你的选项。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-450">Leave **Virtual networks** as **Disabled**, if this is an option for you.</span></span>

    11. <span data-ttu-id="a1f1b-451">单击“创建”。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-451">Click **Create**.</span></span>

        ![填写存储详细信息](images/AzureLabs-Lab313-37.png)

6. <span data-ttu-id="a1f1b-453">一旦你单击**创建**，将需要等待要创建的服务，这可能需要一分钟。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-453">Once you have clicked on **Create**, you will have to wait for the Service to be created, this might take a minute.</span></span>

7. <span data-ttu-id="a1f1b-454">创建服务实例后，在门户中将显示一条通知。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-454">A notification will appear in the Portal once the Service instance is created.</span></span> <span data-ttu-id="a1f1b-455">单击通知以了解新的服务实例。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-455">Click on the notifications to explore your new Service instance.</span></span>

    ![新的存储通知](images/AzureLabs-Lab313-38.png)

8. <span data-ttu-id="a1f1b-457">单击**转到资源**按钮中的通知，和你将转到新的存储服务实例概述页。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-457">Click the **Go to resource** button in the notification, and you will be taken to your new Storage Service instance overview page.</span></span>

    ![转到资源](images/AzureLabs-Lab313-39.png)

9. <span data-ttu-id="a1f1b-459">从概述页上，到右侧，单击**表**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-459">From the overview page, to the right-hand side, click **Tables**.</span></span>
    
    ![表](images/AzureLabs-Lab313-40.png)

10. <span data-ttu-id="a1f1b-461">在右侧面板将更改为显示**表服务**其中所需添加一个新表的信息。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-461">The panel on the right will change to show the **Table Service** information, wherein you need to add a new table.</span></span> <span data-ttu-id="a1f1b-462">执行此操作通过单击 **+ 表**左上角的按钮。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-462">Do this by clicking the **+ Table** button to the top-left corner.</span></span>

    ![打开表](images/AzureLabs-Lab313-41.png)

11. <span data-ttu-id="a1f1b-464">将显示一个新页面，其中你需要输入**表名称**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-464">A new page will be shown, wherein you need to enter a **Table name**.</span></span> <span data-ttu-id="a1f1b-465">这是将用于引用应用程序中后面的章节 （创建函数应用和 Power BI） 的数据的名称。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-465">This is the name you will use to refer to the data in your application in later Chapters (creating Function App, and Power BI).</span></span> <span data-ttu-id="a1f1b-466">插入**IoTMessages**作为名称 （可以选择你自己，只需记住它稍后在本文档中使用时），单击**确定**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-466">Insert **IoTMessages** as the name (you can choose your own, just remember it when used later in this document) and click **OK**.</span></span> 

12. <span data-ttu-id="a1f1b-467">一旦创建新表后，你将能够看到它在**表服务**页 （底部）。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-467">Once the new table has been created, you will be able to see it within the **Table Service** page (at the bottom).</span></span>

    ![创建新表](images/AzureLabs-Lab313-42.png)  

13. <span data-ttu-id="a1f1b-469">现在，单击**访问密钥**，并采取一份**存储帐户名称**并**密钥**（使用在记事本），您将使用这些值更高版本中此课程中，创建时**Azure 函数应用**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-469">Now click on **Access keys** and take a copy of the **Storage account name** and **Key** (using your Notepad), you will use these values later in this course, when creating the **Azure Function App**.</span></span>

    ![创建新表](images/AzureLabs-Lab313-43.png) 

14. <span data-ttu-id="a1f1b-471">同样，在左侧使用面板滚动到*表服务*部分，然后单击**表**(或**浏览表**，较新的门户网站中)，并采取一份**表 URL** （使用在记事本）。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-471">Using the panel on the left again, scroll to the *Table Service* section, and click **Tables** (or **Browse Tables**, in newer Portals) and take a copy of the **Table URL** (using your Notepad).</span></span> <span data-ttu-id="a1f1b-472">链接到您的表时，将稍后在本课程中，使用此值您**Power BI**应用程序。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-472">You will use this value later in this course, when linking your table to your **Power BI** application.</span></span>

    ![创建新表](images/AzureLabs-Lab313-44.png)

## <a name="chapter-12---completing-the-azure-table"></a><span data-ttu-id="a1f1b-474">第 12 章-完成 Azure 表</span><span class="sxs-lookup"><span data-stu-id="a1f1b-474">Chapter 12 - Completing the Azure Table</span></span>

<span data-ttu-id="a1f1b-475">现在，你**表服务**存储帐户已被安装程序，即可将数据添加到它，将用于存储和检索信息。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-475">Now that your **Table Service** storage account has been setup, it is time to add data to it, which will be used to store and retrieve information.</span></span> <span data-ttu-id="a1f1b-476">编辑你的表，可通过完成**Visual Studio**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-476">The editing of your Tables can be done through **Visual Studio**.</span></span>

1. <span data-ttu-id="a1f1b-477">打开**Visual Studio** (**不**Visual Studio Code)。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-477">Open **Visual Studio** (**not** Visual Studio Code).</span></span>

2. <span data-ttu-id="a1f1b-478">从菜单中，单击**视图 > 云资源管理器**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-478">From the menu, click **View > Cloud Explorer**.</span></span>

    ![打开 cloud explorer](images/AzureLabs-Lab313-45.png)

3. <span data-ttu-id="a1f1b-480">**云资源管理器**以停靠项 （如加载可能需要时间，则请耐心等待，） 将打开。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-480">The **Cloud Explorer** will open as a docked item (be patient, as loading may take time).</span></span>

    > [!WARNING] 
    > <span data-ttu-id="a1f1b-481">如果用于创建订阅你*存储帐户*不可见，请确保您具有：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-481">If the subscription you used to create your *Storage Accounts* is not visible, ensure that you have:</span></span> 
    > - <span data-ttu-id="a1f1b-482">登录到 Azure 门户使用的同一个帐户。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-482">Logged in to the same account as the one you used for the Azure Portal.</span></span>
    > - <span data-ttu-id="a1f1b-483">（您可能需要从你的帐户设置应用筛选器） 在帐户管理页上选择你的订阅：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-483">Selected your subscription from the Account Management page (you may need to apply a filter from your account settings):</span></span>  
    >
    >   ![查找订阅](images/AzureLabs-Lab313-46.png)

4. <span data-ttu-id="a1f1b-485">将显示在 Azure 云服务。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-485">Your Azure cloud Services will be shown.</span></span> <span data-ttu-id="a1f1b-486">查找**存储帐户**，然后单击箭头以展开你的帐户的左侧。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-486">Find **Storage Accounts** and click the arrow to the left of that to expand your accounts.</span></span>

    ![打开存储帐户](images/AzureLabs-Lab313-47.png)

5. <span data-ttu-id="a1f1b-488">展开后，新创建**存储帐户**应可用。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-488">Once expanded, your newly created **Storage account** should be available.</span></span> <span data-ttu-id="a1f1b-489">单击你的存储，左侧的箭头，然后后，已展开，找到**表**，然后单击箭头旁边的以显示**表**在最后一章中创建。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-489">Click the arrow to the left of your storage, and then once that is expanded, find **Tables** and click the arrow next to that, to reveal the **Table** you created in the last Chapter.</span></span> <span data-ttu-id="a1f1b-490">双击您**表**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-490">Double-click your **Table**.</span></span>

6. <span data-ttu-id="a1f1b-491">将在 Visual Studio 窗口的中心中打开您的表。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-491">Your table will be opened in the center of your Visual Studio window.</span></span> <span data-ttu-id="a1f1b-492">单击与表图标**+** （加上） 在其上。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-492">Click the table icon with the **+** (plus) on it.</span></span>

    ![添加新表](images/AzureLabs-Lab313-48.png)

7. <span data-ttu-id="a1f1b-494">你能够将出现一个窗口提示*添加实体*。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-494">A window will appear prompting for you to *Add Entity*.</span></span> <span data-ttu-id="a1f1b-495">您将创建一个实体，它将具有三个属性也是如此。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-495">You will create only one entity, though it will have three properties.</span></span> <span data-ttu-id="a1f1b-496">您会注意*PartitionKey*并*RowKey*已提供了，因为需要使用这些表以查找你的数据。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-496">You will notice that *PartitionKey* and *RowKey* are already provided, as these are used by the table to find your data.</span></span> 

    ![分区键和行键](images/AzureLabs-Lab313-49.png)

8. <span data-ttu-id="a1f1b-498">更新以下值：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-498">Update the following values:</span></span>

    - <span data-ttu-id="a1f1b-499">名称：**PartitionKey**，值：**PK_IoTMessages**</span><span class="sxs-lookup"><span data-stu-id="a1f1b-499">Name: **PartitionKey**, Value: **PK_IoTMessages**</span></span> 

    - <span data-ttu-id="a1f1b-500">名称：**RowKey**，值：**RK_1_IoTMessages**</span><span class="sxs-lookup"><span data-stu-id="a1f1b-500">Name: **RowKey**, Value: **RK_1_IoTMessages**</span></span> 

9. <span data-ttu-id="a1f1b-501">然后，单击**将属性添加**(到左下角*添加实体*窗口)，并添加以下属性：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-501">Then, click **Add property** (to the lower left of the *Add Entity* window) and add the following property:</span></span>

    - <span data-ttu-id="a1f1b-502">**MessageContent**，作为*字符串*，将值保留为空。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-502">**MessageContent**, as a *string*, leave the Value empty.</span></span>

10. <span data-ttu-id="a1f1b-503">您的表应与下图中的一个匹配：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-503">Your table should match the one in the image below:</span></span>

    ![添加正确的值](images/AzureLabs-Lab313-50.png)

    > [!NOTE] 
    > <span data-ttu-id="a1f1b-505">实体具有行键中的数字 1 的原因是因为你可能想要添加更多消息，你希望试验进一步与此课程。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-505">The reason why the entity has the number 1 in the row key, is because you might want to add more messages, should you desire to experiment further with this course.</span></span>

11. <span data-ttu-id="a1f1b-506">单击**确定**完成之后。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-506">Click **OK** when you are finished.</span></span> <span data-ttu-id="a1f1b-507">您的表现可供使用。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-507">Your table is now ready to be used.</span></span>

## <a name="chapter-13---create-an-azure-function-app"></a><span data-ttu-id="a1f1b-508">章 13-创建 Azure Function App</span><span class="sxs-lookup"><span data-stu-id="a1f1b-508">Chapter 13 - Create an Azure Function App</span></span> 

<span data-ttu-id="a1f1b-509">现在即可创建*Azure Function App*，这将调用*IoT 中心服务*存储*IoT Edge*设备中的消息**表**服务，在上一章中创建。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-509">It is now time to create an *Azure Function App*, which will be called by the *IoT Hub Service* to store the *IoT Edge* device messages in the **Table** Service, which you created in the previous Chapter.</span></span>

<span data-ttu-id="a1f1b-510">首先，需要创建一个文件，将允许您的 Azure 函数以加载所需的库。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-510">First, you need to create a file that will allow your Azure Function to load the libraries you need.</span></span>

1.  <span data-ttu-id="a1f1b-511">打开**记事本**(按*Windows 键*，然后键入*记事本*)。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-511">Open **Notepad** (press the *Windows Key*, and type *notepad*).</span></span>

    ![打开记事本](images/AzureLabs-Lab313-51.png)

2.  <span data-ttu-id="a1f1b-513">使用记事本打开，将插入到其中的以下 JSON 结构。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-513">With Notepad open, insert the JSON structure below into it.</span></span> <span data-ttu-id="a1f1b-514">完成后，将其保存为您的桌面上**project.json**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-514">Once you have done that, save it on your desktop as **project.json**.</span></span> <span data-ttu-id="a1f1b-515">此文件定义函数将使用的库。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-515">This file defines the libraries your function will use.</span></span> <span data-ttu-id="a1f1b-516">如果已使用 NuGet，它会很熟悉。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-516">If you have used NuGet, it will look familiar.</span></span>
    
    > [!WARNING]
    > <span data-ttu-id="a1f1b-517">很重要的命名正确。请确保它**不具有.txt**文件扩展名。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-517">It is important that the naming is correct; ensure it does **NOT have a .txt** file extension.</span></span> <span data-ttu-id="a1f1b-518">有关参考，请参阅以下内容：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-518">See below for reference:</span></span>
    >
...
3.  <span data-ttu-id="a1f1b-520">登录到[Azure 门户](https://portal.azure.com)。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-520">Log in to the [Azure Portal](https://portal.azure.com).</span></span>

4.  <span data-ttu-id="a1f1b-521">后记录，单击**创建资源**在左上角，并搜索**Function App**，然后按**Enter**键，搜索。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-521">Once you are logged in, click on **Create a resource** in the top left corner, and search for **Function App**, and press the **Enter** key, to search.</span></span> <span data-ttu-id="a1f1b-522">单击*Function App*从结果中，打开一个新的面板。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-522">Click *Function App* from the results, to open a new panel.</span></span>

    ![搜索函数应用](images/AzureLabs-Lab313-53.png)

5.  <span data-ttu-id="a1f1b-524">新的面板将提供的说明**Function App**服务。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-524">The new panel will provide a description of the **Function App** Service.</span></span> <span data-ttu-id="a1f1b-525">在左下角此面板中，单击**创建**按钮，以创建与此关联服务。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-525">At the bottom left of this panel, click the **Create** button, to create an association with this Service.</span></span>

    ![function app 实例](images/AzureLabs-Lab313-54.png)

6.  <span data-ttu-id="a1f1b-527">一旦你单击**创建**，填写以下：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-527">Once you have clicked on **Create**, fill in the following:</span></span>

    1. <span data-ttu-id="a1f1b-528">有关**应用名称**，插入此服务实例所需的名称。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-528">For **App name**, insert your desired name for this Service instance.</span></span>

    2. <span data-ttu-id="a1f1b-529">选择**订阅**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-529">Select a **Subscription**.</span></span>

    3. <span data-ttu-id="a1f1b-530">选择定价层适合你，如果这是首次创建**Function App 服务**，应可供你免费层。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-530">Select the pricing tier appropriate for you, if this is the first time creating a **Function App Service**, a free tier should be available to you.</span></span>

    4. <span data-ttu-id="a1f1b-531">选择**资源组**或新建一个。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-531">Choose a **Resource Group** or create a new one.</span></span> <span data-ttu-id="a1f1b-532">资源组提供了一种方法来监视、 控制访问，预配，以及管理 Azure 资产的集合计费。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-532">A resource group provides a way to monitor, control access, provision, and manage, billing for a collection of Azure assets.</span></span> <span data-ttu-id="a1f1b-533">建议将所有 Azure 服务与常见的资源组下的单个项目 （例如如这些课程））。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-533">It is recommended to keep all the Azure Services associated with a single project (e.g. such as these courses) under a common resource group).</span></span>

        > <span data-ttu-id="a1f1b-534">如果你想要阅读更多有关 Azure 资源组，请遵循此[如何管理资源组链接](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-portal)。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-534">If you wish to read more about Azure Resource Groups, please follow this [link on how to manage a Resource Group](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-portal).</span></span>

    5. <span data-ttu-id="a1f1b-535">有关**OS**，单击 Windows，因为这是预期的平台。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-535">For **OS**, click Windows, as that is the intended platform.</span></span>

    6. <span data-ttu-id="a1f1b-536">选择**托管计划**(使用本教程**消耗计划**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-536">Select a **Hosting Plan** (this tutorial is using a **Consumption Plan**.</span></span>

    7. <span data-ttu-id="a1f1b-537">选择**位置**（上一步中选择与已生成的存储相同的位置）</span><span class="sxs-lookup"><span data-stu-id="a1f1b-537">Select a **Location** (choose the same location as the storage you have built in the previous step)</span></span>

    8. <span data-ttu-id="a1f1b-538">有关**存储**部分中，**必须选择在上一步中创建的存储服务**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-538">For the **Storage** section, **you must select the Storage Service you created in the previous step**.</span></span>

    9. <span data-ttu-id="a1f1b-539">您不需要*Application Insights*在此应用中，所以请随意将其留**关闭**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-539">You will not need *Application Insights* in this app, so feel free to leave it **Off**.</span></span>

    10. <span data-ttu-id="a1f1b-540">单击“创建”。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-540">Click **Create**.</span></span>

        ![创建新的实例](images/AzureLabs-Lab313-55.png)

7.  <span data-ttu-id="a1f1b-542">一旦你单击**创建**，将需要等待要创建的服务，这可能需要一分钟。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-542">Once you have clicked on **Create**, you will have to wait for the Service to be created, this might take a minute.</span></span>

8.  <span data-ttu-id="a1f1b-543">创建服务实例后，在门户中将显示一条通知。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-543">A notification will appear in the Portal once the Service instance is created.</span></span>

    ![新的通知](images/AzureLabs-Lab313-56.png)

9.  <span data-ttu-id="a1f1b-545">部署成功后单击该通知 （已完成）。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-545">Click on the notification, once deployment is successful (has finished).</span></span>

10. <span data-ttu-id="a1f1b-546">单击**转到资源**通知探索新的服务实例中的按钮。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-546">Click the **Go to resource** button in the notification to explore your new Service instance.</span></span> 

    ![转到资源](images/AzureLabs-Lab313-57.png)

11. <span data-ttu-id="a1f1b-548">在左侧和右侧的新面板，单击**+** （加号） 旁边的图标*函数*，以创建新的函数。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-548">In the left side of the new panel, click the **+** (plus) icon next to *Functions*, to create a new function.</span></span>

    ![添加新函数](images/AzureLabs-Lab313-58.png)

12. <span data-ttu-id="a1f1b-550">在中间面板**函数**创建窗口将显示。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-550">Within the central panel, the **Function** creation window will appear.</span></span> <span data-ttu-id="a1f1b-551">此外，向下滚动并单击**自定义函数**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-551">Scroll down further, and click on **Custom function**.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-59.png)

13. <span data-ttu-id="a1f1b-553">下一个页面，直到找到中向下的滚动**IoT 中心 （事件中心）**，然后单击它。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-553">Scroll down the next page, until you find **IoT Hub (Event Hub)**, then click on it.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-60.png)

14. <span data-ttu-id="a1f1b-555">在中**IoT 中心 （事件中心）** 边栏选项卡，将**语言**到**C#** ，然后单击**新**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-555">In the **IoT Hub (Event Hub)** blade, set the **Language** to **C#** and then click on **new**.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-61.png)

15. <span data-ttu-id="a1f1b-557">在窗口中将出现，请确保**IoT 中心**处于选中状态和名称*IoT 中心*字段对应的名称与你*IoT 中心服务*必须以前创建 ([在步骤 8 的第 3 章](#chapter-3---the-iot-hub-service))。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-557">In the window that will appear, make sure that **IoT Hub** is selected and the name of the *IoT Hub* field corresponds with the name of your *IoT Hub Service* that you have created previously ([in step 8, of Chapter 3](#chapter-3---the-iot-hub-service)).</span></span> <span data-ttu-id="a1f1b-558">然后单击**选择**按钮。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-558">Then click the **Select** button.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-62.png)

16. <span data-ttu-id="a1f1b-560">重新**IoT 中心 （事件中心）** 边栏选项卡上，单击**创建**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-560">Back on the **IoT Hub (Event Hub)** blade, click on **Create**.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-63.png)

17. <span data-ttu-id="a1f1b-562">你将重定向到在函数编辑器。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-562">You will be redirected to the function editor.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-64.png)

18. <span data-ttu-id="a1f1b-564">删除在其中的所有代码并使用以下代码替换它：</span><span class="sxs-lookup"><span data-stu-id="a1f1b-564">Delete all the code in it and replace it with the following:</span></span>
...
19. <span data-ttu-id="a1f1b-565">更改以下变量，以便它们对应于相应的值 (**表**并**存储**值，从[步骤 11 到 13，分别，第 11 章](#chapter-11---create-table-service))，将在中找到你**存储帐户**:</span><span class="sxs-lookup"><span data-stu-id="a1f1b-565">Change the following variables, so that they correspond to the appropriate values (**Table** and **Storage** values, from [step 11 and 13, respectively, of Chapter 11](#chapter-11---create-table-service)), that you will find in your **Storage Account**:</span></span>

    - <span data-ttu-id="a1f1b-566">**tableName**，名称为你**表**位于你**存储帐户**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-566">**tableName**, with the name of your **Table** located in your **Storage Account**.</span></span>
    - <span data-ttu-id="a1f1b-567">**tableURL**，url 为你**表**位于你**存储帐户**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-567">**tableURL**, with the URL of your **Table** located in your **Storage Account**.</span></span>
    - <span data-ttu-id="a1f1b-568">**storageAccountName**，具有的名称相对应的值的名称与你**存储帐户**名称。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-568">**storageAccountName**, with the name of the value corresponding with the name of your **Storage Account** name.</span></span>
    - <span data-ttu-id="a1f1b-569">**storageAccountKey**，已在具有以前创建的存储服务中获取的密钥。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-569">**storageAccountKey**, with the Key you have obtained in the Storage Service you have created previously.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-65.png)

20. <span data-ttu-id="a1f1b-571">使用的代码中的位置，单击**保存**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-571">With the code in place, click **Save**.</span></span>

21. <span data-ttu-id="a1f1b-572">接下来，单击**\<** （箭头） 图标，在页面的右侧。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-572">Next, click the **\<** (arrow) icon, on the right-hand side of the page.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-66.png)

22. <span data-ttu-id="a1f1b-574">一个面板，将从右侧滑中。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-574">A panel will slide in from the right.</span></span> <span data-ttu-id="a1f1b-575">在该面板中，单击**上传**，和一个*文件浏览器*将出现。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-575">In that panel, click **Upload**, and a *File Browser* will appear.</span></span>

23. <span data-ttu-id="a1f1b-576">导航到，然后单击， **project.json**中创建的文件**记事本**以前，然后单击**打开**按钮。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-576">Navigate to, and click, the **project.json** file, which you created in **Notepad** previously, and then click the **Open** button.</span></span> <span data-ttu-id="a1f1b-577">此文件定义函数将使用的库。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-577">This file defines the libraries that your function will use.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-67.png)

24. <span data-ttu-id="a1f1b-579">当完成上传文件时，它会在右侧面板中显示。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-579">When the file has uploaded, it will appear in the panel on the right.</span></span> <span data-ttu-id="a1f1b-580">单击它即可打开它内**函数**编辑器。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-580">Clicking it will open it within the **Function** editor.</span></span> <span data-ttu-id="a1f1b-581">它必须看起来**完全**的下一步的映像相同。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-581">It must look **exactly** the same as the next image.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-68.png)

25. <span data-ttu-id="a1f1b-583">现在那会很不错，若要测试你的函数的功能，若要将该消息存储在你*表*。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-583">At this point it would be good to test the capability of your Function to store the message on your *Table*.</span></span> <span data-ttu-id="a1f1b-584">在窗口右上角，单击**测试**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-584">On the top right side of the window, click on **Test**.</span></span>

    ![自定义函数](images/AzureLabs-Lab313-69.png)

26. <span data-ttu-id="a1f1b-586">插入一条消息上**请求正文**，在上图中的，然后单击中所示**运行**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-586">Insert a message on the **Request body**, as shown in the image above, and click on **Run**.</span></span> 

27. <span data-ttu-id="a1f1b-587">该函数将运行，并显示结果状态 (您将注意到绿色**状态 202 已接受**上面*输出*窗口中，这意味着它已成功调用):</span><span class="sxs-lookup"><span data-stu-id="a1f1b-587">The function will run, displaying the result status (you will notice the green **Status 202 Accepted**, above the *Output* window, which means it was a successful call):</span></span>

    ![输出结果](images/AzureLabs-Lab313-70.png)

## <a name="chapter-14---view-active-messages"></a><span data-ttu-id="a1f1b-589">第 14 章 — 查看活动消息</span><span class="sxs-lookup"><span data-stu-id="a1f1b-589">Chapter 14 - View active messages</span></span>

<span data-ttu-id="a1f1b-590">如果现在打开 Visual Studio (**不**Visual Studio Code)，可以可视化您测试的消息结果，因为它将存储在*MessageContent*区域的字符串。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-590">If you now open Visual Studio (**not** Visual Studio Code), you can visualize your test message result, as it will be stored in the *MessageContent* string area.</span></span>

![自定义函数](images/AzureLabs-Lab313-71.png)

<span data-ttu-id="a1f1b-592">使用表服务和 Function App 中的位置，在 Ubuntu 设备的消息将出现在你*IoTMessages*表。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-592">With the Table Service and Function App in place, your Ubuntu device messages will appear in your *IoTMessages* Table.</span></span> <span data-ttu-id="a1f1b-593">如果尚未运行，再次启动你的设备，您将能够看到的结果消息从设备和模块，在表中，使用 Visual Studio*云资源管理器*。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-593">If not already running, start your device again, and you will be able to see the result messages from your device, and module, within your Table, through using Visual Studio *Cloud Explorer*.</span></span>

![实现数据可视化效果](images/AzureLabs-Lab313-72.png)


## <a name="chapter-15---power-bi-setup"></a><span data-ttu-id="a1f1b-595">第 15 章-Power BI 安装程序</span><span class="sxs-lookup"><span data-stu-id="a1f1b-595">Chapter 15 - Power BI Setup</span></span>

<span data-ttu-id="a1f1b-596">若要从 IOT 设备将在安装程序将数据可视化**Power BI** （桌面版），以收集数据的*表*刚创建的服务。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-596">To visualize the data from your IOT device you will setup **Power BI** (desktop version), to collect the data from the *Table* Service, which you just created.</span></span> <span data-ttu-id="a1f1b-597">*HoloLens*版本的 Power BI 然后会使用该数据的结果可视化。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-597">The *HoloLens* version of Power BI will then use that data to visualize the result.</span></span>

1.  <span data-ttu-id="a1f1b-598">打开 Windows 10 上的 Microsoft Store 并搜索**Power BI Desktop**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-598">Open the Microsoft Store on Windows 10 and search for **Power BI Desktop**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-73.png)

2.  <span data-ttu-id="a1f1b-600">下载应用程序。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-600">Download the application.</span></span> <span data-ttu-id="a1f1b-601">完成下载后，请将其打开。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-601">Once it has finished downloading, open it.</span></span>

3.  <span data-ttu-id="a1f1b-602">登录到*Power BI*与你**Microsoft 365 帐户**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-602">Log into *Power BI* with your **Microsoft 365 account**.</span></span> <span data-ttu-id="a1f1b-603">你可能会重定向到浏览器中，若要注册。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-603">You may be redirected to a browser, to sign up.</span></span> <span data-ttu-id="a1f1b-604">一旦您注册了，请返回到 Power BI 应用，并再次登录。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-604">Once you are signed up, go back to the Power BI app, and sign in again.</span></span>

4.  <span data-ttu-id="a1f1b-605">单击**获取数据**，然后单击**更多...**.</span><span class="sxs-lookup"><span data-stu-id="a1f1b-605">Click on **Get Data** and then click on **More...**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-74.png)

5.  <span data-ttu-id="a1f1b-607">单击**Azure**， **Azure 表存储**，然后单击**Connect**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-607">Click **Azure**, **Azure Table Storage**, then click on **Connect**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-75.png)

6.  <span data-ttu-id="a1f1b-609">系统会提示要插入**表 URL**先前收集的 ([在步骤 13 第 11 章](#chapter-11---create-table-service))，创建表服务时。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-609">You will be prompted to insert the **Table URL** that you collected earlier ([in step 13 of Chapter 11](#chapter-11---create-table-service)), while creating your Table Service.</span></span> <span data-ttu-id="a1f1b-610">插入 URL 后, 删除引用表"子文件夹"（这是 IoTMessages，本课程中） 的路径的一部分。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-610">After inserting the URL, delete the portion of the path referring to the Table "sub-folder" (which was IoTMessages, in this course).</span></span> <span data-ttu-id="a1f1b-611">最终结果应如下图中所示。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-611">The final result should be as displayed in the image below.</span></span> <span data-ttu-id="a1f1b-612">然后单击**确定**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-612">Then click on **OK**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-76.png)

7.  <span data-ttu-id="a1f1b-614">系统会提示要插入**存储密钥**记下 ([在第 11 章第 11 步](#chapter-11---create-table-service)) 早期创建在表存储时。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-614">You will be prompted to insert the **Storage Key** that you noted ([in step 11 of Chapter 11](#chapter-11---create-table-service)) earlier while creating your Table Storage.</span></span> <span data-ttu-id="a1f1b-615">然后单击**Connect**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-615">Then click on **Connect**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-77.png)  

8. <span data-ttu-id="a1f1b-617">一个**导航器面板**将显示，选中你的表旁边的框，然后单击**负载**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-617">A **Navigator Panel** will be displayed, tick the box next to your Table and click on **Load**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-78.png)  

9. <span data-ttu-id="a1f1b-619">现在在 Power BI 上加载您的表，但它需要一个查询来显示中它的值。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-619">Your table has now been loaded on Power BI, but it requires a query to display the values in it.</span></span> <span data-ttu-id="a1f1b-620">为此，请右键单击表名称中位于**字段面板**在屏幕的右侧。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-620">To do so, right-click on the table name located in the **FIELDS panel** at the right side of the screen.</span></span> <span data-ttu-id="a1f1b-621">然后单击**编辑查询**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-621">Then click on **Edit Query**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-79.png) 

10. <span data-ttu-id="a1f1b-623">一个**Power Query 编辑器**以显示您的表的新窗口将打开。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-623">A **Power Query Editor**  will open up as a new window, displaying your table.</span></span> <span data-ttu-id="a1f1b-624">单击单词**记录**内*内容*表，可视化存储的内容的列。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-624">Click on the word **Record** within the *Content* column of the table, to visualize your stored content.</span></span>

    ![Power BI](images/AzureLabs-Lab313-80.png)    

11. <span data-ttu-id="a1f1b-626">单击**Into 表**，在窗口的左上方。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-626">Click on **Into Table**, at the top-left of the window.</span></span> 

    ![Power BI](images/AzureLabs-Lab313-81.png)

12. <span data-ttu-id="a1f1b-628">单击**关闭并应用**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-628">Click on **Close & Apply**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-82.png)

13. <span data-ttu-id="a1f1b-630">在加载查询，完成**字段面板**，在屏幕的右侧，勾选对应参数的框**名称**并**值**到可视化**MessageContent**列的内容。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-630">Once it has finished loading the query, within the **FIELDS panel**, on the right side of the screen, tick the boxes corresponding to the parameters **Name** and **Value**, to visualize the **MessageContent** column content.</span></span>

    ![Power BI](images/AzureLabs-Lab313-83.png)

14. <span data-ttu-id="a1f1b-632">单击**蓝色磁盘图标**在左上角的窗口在所选的文件夹中保存工作。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-632">Click on the **blue disk icon** at the top left of the window to save your work in a folder of your choice.</span></span>

    ![Power BI](images/AzureLabs-Lab313-84.png)

15. <span data-ttu-id="a1f1b-634">现在可以单击发布按钮以将您的表上传到你的工作区。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-634">You can now click on the Publish button to upload your table to your Workspace.</span></span> <span data-ttu-id="a1f1b-635">出现提示时，单击**我的工作区**然后单击*选择*。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-635">When prompted, click **My workspace** and click *Select*.</span></span> <span data-ttu-id="a1f1b-636">等待显示成功提交的结果。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-636">Wait for it to display the successful result of the submission.</span></span>
...
> [!WARNING]
> <span data-ttu-id="a1f1b-639">以下章节都是特定的 HoloLens。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-639">The following Chapter is HoloLens specific.</span></span> <span data-ttu-id="a1f1b-640">Power BI 当前不提供为沉浸式应用程序，但可以在 Windows 混合现实门户 （也称为 Cliff 房屋） 中运行的桌面版本通过桌面应用程序。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-640">Power BI is not currently availble as an immersive application, however you can run the desktop version in the Windows Mixed Reality Portal (aka Cliff House), through the Desktop app.</span></span>

## <a name="chapter-16---display-power-bi-data-on-hololens"></a><span data-ttu-id="a1f1b-641">第 16 章 — 在 HoloLens 上显示 Power BI 数据</span><span class="sxs-lookup"><span data-stu-id="a1f1b-641">Chapter 16 - Display Power BI data on HoloLens</span></span>

1. <span data-ttu-id="a1f1b-642">在你 HoloLens 登录**Microsoft Store**，通过点击应用程序列表中对应的图标。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-642">On your HoloLens, log in to the **Microsoft Store**, by tapping on its icon in the applications list.</span></span>

    ![Power BI HL](images/AzureLabs-Lab313-87.png)

2. <span data-ttu-id="a1f1b-644">搜索，然后下载**Power BI**应用程序。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-644">Search and then download the **Power BI** application.</span></span>

    ![Power BI HL](images/AzureLabs-Lab313-88.png)

3. <span data-ttu-id="a1f1b-646">启动**Power BI**从您的应用程序的列表。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-646">Start **Power BI** from your applications list.</span></span> 

4. <span data-ttu-id="a1f1b-647">**Power BI**可能会要求你登录到你**Microsoft 365 帐户**。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-647">**Power BI** might ask you to login to your **Microsoft 365 account**.</span></span>

5. <span data-ttu-id="a1f1b-648">一次在应用程序，在工作区应显示默认情况下下, 图中所示。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-648">Once inside the app, the workspace should display by default as shown in the image below.</span></span> <span data-ttu-id="a1f1b-649">如果不会发生，只需单击窗口左侧的工作区图标。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-649">If that does not happen, simply click on the workspace icon on the left side of the window.</span></span>

    ![Power BI HL](images/AzureLabs-Lab313-89.png)

## <a name="your-finished-your-iot-hub-application"></a><span data-ttu-id="a1f1b-651">在完成 IoT 中心应用程序</span><span class="sxs-lookup"><span data-stu-id="a1f1b-651">Your finished your IoT Hub application</span></span>

<span data-ttu-id="a1f1b-652">祝贺您，您已成功创建 IoT 中心服务，使用模拟的虚拟机 Edge 设备。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-652">Congratulations, you have successfully created an IoT Hub Service, with a simulated Virtual Machine Edge device.</span></span> <span data-ttu-id="a1f1b-653">你的设备可以进行通信机器学习模型到 Azure 表服务，利用 Azure Function App，这是读取到 Power BI，Microsoft HoloLens 中可视化的结果。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-653">Your device can  communicate the results of a machine learning model to an Azure Table Service, facilitated by an Azure Function App, which is read into Power BI, and visualized within a Microsoft HoloLens.</span></span>
 
![Power BI](images/AzureLabs-Lab313-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="a1f1b-655">Bonus 练习</span><span class="sxs-lookup"><span data-stu-id="a1f1b-655">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="a1f1b-656">练习 1</span><span class="sxs-lookup"><span data-stu-id="a1f1b-656">Exercise 1</span></span>

<span data-ttu-id="a1f1b-657">展开表中存储的消息传递结构并将其显示为图形。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-657">Expand the messaging structure stored in the table and display it as a graph.</span></span> <span data-ttu-id="a1f1b-658">你可能想要收集更多数据并将其存储在同一个表，以更高版本显示。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-658">You might want to collect more data and store it in the same table, to be later displayed.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="a1f1b-659">练习 2</span><span class="sxs-lookup"><span data-stu-id="a1f1b-659">Exercise 2</span></span>

<span data-ttu-id="a1f1b-660">创建一个附加"相机捕捉"模块来部署 IoT 板上，以便它可以捕获映像通过摄像机来对其进行分析。</span><span class="sxs-lookup"><span data-stu-id="a1f1b-660">Create an additional "camera capture" module to be deployed on the IoT board, so that it can capture images through the camera to be analyzed.</span></span>
