...
* **设置**:控制通过检查以下设置捕获媒体流：
  ...
  * **Mic 音频**:捕获音频从麦克风阵列。
  * **应用音频**:从当前正在运行的应用捕获音频。
  * **从照相机呈现**:对齐捕获照片/视频摄像机视角为如果[支持的正在运行的应用](mixed-reality-capture-for-developers.md#render-from-the-pv-camera-opt-in)(仅 HoloLens 2)。
...
> [!NOTE]
> 有[限制同时 MRC](mixed-reality-capture-for-developers.md#simultaneous-mrc-limitations):
> * 如果应用程序尝试访问照片/视频照相机，而 Windows Device Portal 录制视频时，将停止视频录制。
>   * HoloLens 2 将不会停止录制视频，如果应用 acesses 照片/视频照相机 SharedReadOnly 模式。
> * 如果应用正在主动使用照片/视频摄像机，就能够拍摄照片或视频记录 Windows Device Portal。
> * 实时传送视频流：
>   * HoloLens （第 1 代） 将阻止应用在实时流式处理从 Windows Device Portal 访问照片/视频摄像机。
>   * HoloLens （第 1 代） 将无法实时流，如果应用正在主动使用照片/视频摄像机。
>   * HoloLens 2 将自动停止实时传送视频流应用程序尝试访问 ExclusiveControl 模式中的照片/视频照相机时。
>   * HoloLens 2 可启动实时流，同时应用正在主动使用 PV 照相机。
