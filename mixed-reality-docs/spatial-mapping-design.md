...
为了提供最佳的用户体验, 请务必了解影响 HoloLens 收集的空间映射数据质量的因素。
...
几个因素可能会影响这些错误的频率和严重性:

* **用户运动**
   * 用户在其环境中的移动方式决定了环境的扫描情况, 因此用户可能需要指导才能实现良好的扫描。
   * 用于扫描的相机在70度的圆锥范围内提供数据, 从最小0.8 米到最大3.1 米距离相机。 实际的图面将仅在此视图字段内扫描。 请注意, 这些值在将来的版本中可能会更改。
   * 如果用户永远不会获得对象的3.1 米以内, 则不会对其进行扫描。
   * 如果用户仅查看小于0.8 米的距离的对象, 则不会对其进行扫描 (这将避免扫描用户的手)。
   * 如果用户永不向上查看 (相当正常), 则可能不会扫描天花板。
   * 如果用户不会在家具或墙壁后面显示, 则不会扫描封闭像素的对象。
   * 在表面上查看时, 表面的扫描速度会更高, 而不是在浅时扫描。
   * 如果 HoloLens 的打印头跟踪系统很快就会失败 (这可能是由于快速的用户动作、光照不佳、featureless 墙壁或相机被覆盖而导致的), 这可能会导致空间映射数据出错。 当用户继续移动和扫描其环境时, 任何此类错误都会得到更正。

* **Surface 材料**
   * 实际图面上发现的材料差别很大。 这会影响空间映射数据的质量, 因为它们会影响红外光源的反射方式。
   * 暗表面可能无法扫描, 直到它们接近相机, 因为它们的光线更小。
   * 某些表面可能会很暗, 它们反映出的光线太少, 无法从任何距离进行扫描, 因此, 它们会在表面位置和曲面后面的位置引入孔错误。
   * 特别有光泽的表面只能在观看时扫描, 而不能在从浅角度观看时扫描。
   * 镜像, 因为它们创建了实空格和曲面的 illusory 反射, 因此可能导致孔洞错误和 hallucination 错误。

* **场景运动**
   * 空间映射可以对环境中的更改 (例如移动人员或打开和结束门) 进行快速调整。
   * 但是, 只有当区域对用于扫描的照相机清晰可见时, 空间映射才能适应区域中的更改。
   * 因此, 这种情况可能会使这种调整滞后于现实, 这可能会导致洞或 hallucination 错误。
   * 例如, 用户扫描朋友, 然后在朋友离开房间时翻过来。 朋友的 "ghost" 表示形式 (hallucination 错误) 将保留在空间映射数据中, 直到用户返回并重新扫描朋友所在的空间。

* **光照干扰**
   * 场景中的环境红外线灯可能会干扰扫描, 例如, 通过窗口进入强大的阳光。
   * 特别有光泽的图面可能会干扰附近表面的扫描, 而光线会导致偏移错误。
   * 视频面直接将光反射回相机可能会干扰附近的 hallucinations, 也可能会导致浮动的中端, 或延迟调整到场景运动。
   * 同一房间中的两个 HoloLens 设备不应相互干扰, 但有超过5个 HoloLens 设备可能会造成干扰。

对于其中一些错误, 可能会避免或更正此问题。 但是, 你应设计应用程序, 以便即使在空间映射数据中存在错误时, 用户也能够实现其目标。

## <a name="the-environment-scanning-experience"></a>环境扫描体验

HoloLens 在用户查看其环境中的表面时了解到它们。 随着时间的推移, HoloLens 会生成一个扫描环境中所有部件的扫描。 它还会在观察到环境中的更改时更新扫描。 此扫描将在应用启动时自动保留。
...
* **无扫描体验**
   ...
   * 例如, 允许用户在包含全息 spraypaint 的表面上绘图的应用程序只需了解当前对用户可见的表面。
