...
>[!NOTE]
> Update () 是此性能问题的最常见表现形式, 但如果不糟糕, 其他重复 Unity 回调 (如以下) 可能会同样糟糕:FixedUpdate ()、Behaviour ()、OnPostRender "、OnPreRender ()、OnRenderImage () 等。 
...
使用*着色器预加载*和其他技巧优化[着色器的加载时间](http://docs.unity3d.com/Manual/OptimizingShaderLoadTime.html)。 特别是, 着色器预加载意味着你不会看到任何 hitches, 因为运行时着色器编译。
