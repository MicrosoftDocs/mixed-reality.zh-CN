...
3.  在左侧面板中展开**保存的数据集** > **我的数据集**并将其拖**ProductsTableCSV**到**试验画布**.
...
12. 从左侧的试验项目面板中，展开**机器学习** > **初始化模型** > **分类**，并将其拖**多类逻辑回归**到试验画布上的项。
...
3.  接下来，请转到**文件** > **生成设置**，并切换到平台**通用 Windows 平台**，通过单击***切换平台***按钮。
...
3.  选中复选框调用**UnityC#项目**（这是重要因为这可以使您可以编辑类生成完成后）。
