...
3.  在左侧的展开的面板从 **保存的数据集*> * 我的数据集 * * 和拖动**ProductsTableCSV**到**试验画布**。
...
7.  在中**试验**左侧和右侧面板中，展开 **机器学习*> * 训练 **。拖动 **训练模型 * * 中的项扩展到试验画布。 画布应为相同的外观如下。
...
12. 从左侧的试验项目面板中，展开 * **机器学习* > *初始化模型*> * 分类 ***，并将其拖 **多类逻辑回归 * * 项到试验画布上。

13. 连接输出中，从底部**多类逻辑回归**，到的顶部左侧输入**训练模型**项。

    ![机器学习工作室：实验](images/AzureLabs-Lab7-23.png)

14. 在左侧面板中的试验项目的列表中，展开 **机器学习*> * 分数 **，并将其拖 **分数模型 * * 到画布上的项。
...
17. 在列表中**实验**在左侧面板中的项展开 * **机器学习*> * 评估 ***，并将其拖 **评估模型 * * 项拖动到画布上的。
