...
恭喜你！ 应用现在正在使用 Windows 10 通用 Windows 平台（UWP）。
...
HoloLens 使用高级深度传感器来查看世界并看到用户。 这样就可以实现先进的手势，如[布隆](system-gesture.md#bloom)和[分流](gaze-and-commit.md#composite-gestures)。 强大的麦克风还启用了[语音体验](voice-input.md)。

使用桌面耳机，用户可以使用运动控制器指向应用并采取措施。 他们还可以使用游戏板，将对象定位在其看板上。

Windows 负责处理 UWP 应用的所有复杂性，将[注视](gaze-and-commit.md)、手势、语音和运动控制器输入转换为抽象出输入机制的[指针事件](https://msdn.microsoft.com/library/windows/apps/mt404610#pointer_events)。 例如，用户可能已在运动控制器上进行了一次轻点击，或在运动控制器上拉取了 Select 触发器，但2D 应用程序无需知道输入来自何处-它们只是在触摸屏上看到2D 触摸按下。

下面是你在将 UWP 应用引入 HoloLens 时应了解的高级概念/方案：
* [注视](gaze-and-commit.md)悬停事件，这可能会意外触发菜单、浮出控件或其他用户界面元素，只需 gazing 围绕应用程序进行弹出。
