...
## <a name="before-you-start"></a><span data-ttu-id="7d1c5-151">开始之前</span><span class="sxs-lookup"><span data-stu-id="7d1c5-151">Before you start</span></span>
...
6.  <span data-ttu-id="7d1c5-237">在*生成设置*窗口中，单击**播放器设置**按钮，这将打开相关面板中的空间中的*检查器*所在。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-237">In the *Build Settings* window, click on the **Player Settings** button, this will open the related panel in the space where the *Inspector* is located.</span></span>
...
## <a name="chapter-4---importing-the-newtonsoft-dll-in-unity"></a><span data-ttu-id="7d1c5-251">第 4 章-导入 Unity 中 Newtonsoft DLL</span><span class="sxs-lookup"><span data-stu-id="7d1c5-251">Chapter 4 - Importing the Newtonsoft DLL in Unity</span></span>
...
7.  <span data-ttu-id="7d1c5-301">为代码**Awake()** 现在需要添加初始化该实例变量：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-301">Code for **Awake()** now needs to be added to initialize the Instance variable:</span></span>
...
9.  <span data-ttu-id="7d1c5-357">添加**SubmitImageForTraining()** 方法。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-357">Add the **SubmitImageForTraining()** method.</span></span> <span data-ttu-id="7d1c5-358">此方法将开始自定义影像服务训练过程。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-358">This method will begin the Custom Vision Service training process.</span></span> <span data-ttu-id="7d1c5-359">第一步是检索**标记 Id**从与用户的已验证的语音输入相关联的服务。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-359">The first step is to retrieve the **Tag Id** from the Service which is associated with the validated speech input from the user.</span></span> <span data-ttu-id="7d1c5-360">**标记 Id**然后以及映像上传。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-360">The **Tag Id** will then be uploaded along with the image.</span></span>
...
13. <span data-ttu-id="7d1c5-371">此类中添加的最后一个方法是**GetImageAsByteArray()** 方法，用于在 web 调用要将映像捕获到的字节数组转换。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-371">The last method to add in this class is the **GetImageAsByteArray()** method, used on the web calls to convert the image captured into a byte array.</span></span>
...
14. <span data-ttu-id="7d1c5-372">请务必保存中所做的更改**Visual Studio**才会回到**Unity**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-372">Be sure to save your changes in **Visual Studio** before returning to **Unity**.</span></span>

## <a name="chapter-10---create-the-sceneorganiser-class"></a><span data-ttu-id="7d1c5-373">章 10-创建 SceneOrganiser 类</span><span class="sxs-lookup"><span data-stu-id="7d1c5-373">Chapter 10 - Create the SceneOrganiser class</span></span>

<span data-ttu-id="7d1c5-374">将此类：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-374">This class will:</span></span>

-   <span data-ttu-id="7d1c5-375">创建**游标**要附加到 Main Camera 对象。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-375">Create a **Cursor** object to attach to the Main Camera.</span></span>

-   <span data-ttu-id="7d1c5-376">创建**标签**对象，该服务识别出的实际对象时，将显示对象。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-376">Create a **Label** object that will appear when the Service recognizes the real-world objects.</span></span>

-   <span data-ttu-id="7d1c5-377">通过附加到适当的组件设置 Main Camera。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-377">Set up the Main Camera by attaching the appropriate components to it.</span></span>

-   <span data-ttu-id="7d1c5-378">在何时**分析模式**、 生成的标签，在运行时，相对于主相机的位置的适当世界空间中并显示从自定义影像服务接收的数据。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-378">When in **Analysis Mode**, spawn the Labels at runtime, in the appropriate world space relative to the position of the Main Camera, and display the data received from the Custom Vision Service.</span></span>

-   <span data-ttu-id="7d1c5-379">在何时**培训模式**，生成将显示在定型过程的不同阶段的 UI。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-379">When in **Training Mode**, spawn the UI that will display the different stages of the training process.</span></span>

<span data-ttu-id="7d1c5-380">若要创建此类：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-380">To create this class:</span></span>

1.  <span data-ttu-id="7d1c5-381">内右击**脚本**文件夹，然后单击**创建** > **C\#脚本**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-381">Right-click inside the **Scripts** folder, then click **Create** > **C\# Script**.</span></span> <span data-ttu-id="7d1c5-382">脚本命名为*SceneOrganiser*。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-382">Name the script *SceneOrganiser*.</span></span>

2.  <span data-ttu-id="7d1c5-383">双击新*SceneOrganiser*脚本以将其与打开**Visual Studio**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-383">Double-click on the new *SceneOrganiser* script to open it with **Visual Studio**.</span></span>

3.  <span data-ttu-id="7d1c5-384">您只需要一个命名空间，删除上面提供的其他*SceneOrganiser*类：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-384">You will only need one namespace, remove the others from above the *SceneOrganiser* class:</span></span>

    ```csharp
    using UnityEngine;
    ```

4.  <span data-ttu-id="7d1c5-385">然后添加以下变量内的*SceneOrganiser*类，更高版本**start （)** 方法：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-385">Then add the following variables inside the *SceneOrganiser* class, above the **Start()** method:</span></span>
...
5.  <span data-ttu-id="7d1c5-386">删除**start （)** 并**update （)** 方法。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-386">Delete the **Start()** and **Update()** methods.</span></span>

6.  <span data-ttu-id="7d1c5-387">右下方变量中，添加**Awake()** 方法，它将初始化类并设置场景。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-387">Right underneath the variables, add the **Awake()** method, which will initialize the class and set up the scene.</span></span>
...
7.  <span data-ttu-id="7d1c5-388">现在，添加**CreateCameraCursor()** 方法，用于创建将 Main Camera 光标定位并**CreateLabel()** 方法，用于创建**分析标签**对象.</span><span class="sxs-lookup"><span data-stu-id="7d1c5-388">Now add the **CreateCameraCursor()** method that creates and positions the Main Camera cursor, and the **CreateLabel()** method, which creates the **Analysis Label** object.</span></span>
...
8. <span data-ttu-id="7d1c5-389">添加**SetCameraStatus()** 方法，它将处理适用于文本网格提供的相机的状态消息。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-389">Add the **SetCameraStatus()** method, which will handle messages intended for the text mesh providing the status of the camera.</span></span>
...
9. <span data-ttu-id="7d1c5-390">添加**PlaceAnalysisLabel()** 并**SetTagsToLastLabel()** 方法，这将生成并向场景中显示自定义影像服务中的数据。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-390">Add the **PlaceAnalysisLabel()** and **SetTagsToLastLabel()** methods, which will spawn and display the data from the Custom Vision Service into the scene.</span></span>
...
10. <span data-ttu-id="7d1c5-391">最后，将添加**CreateTrainingUI()** 方法，将生成应用程序时在定型模式下都显示在定型过程的多个阶段的 UI。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-391">Lastly, add the **CreateTrainingUI()** method, which will spawn the UI displaying the multiple stages of the training process when the application is in Training Mode.</span></span> <span data-ttu-id="7d1c5-392">此方法还会利用创建相机状态对象。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-392">This method will also be harnessed to create the camera status object.</span></span>
...
11. <span data-ttu-id="7d1c5-393">请务必保存中所做的更改**Visual Studio**才会回到**Unity**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-393">Be sure to save your changes in **Visual Studio** before returning to **Unity**.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="7d1c5-394">在继续之前，打开**CustomVisionAnalyser**类，并在**AnalyseLastImageCaptured()** 方法，*取消注释*以下行：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-394">Before you continue, open the **CustomVisionAnalyser** class, and within the **AnalyseLastImageCaptured()** method, *uncomment* the following lines:</span></span>
>
> ```csharp
>   AnalysisObject analysisObject = new AnalysisObject();
>   analysisObject = JsonConvert.DeserializeObject<AnalysisObject>(jsonResponse);
...
## <a name="chapter-11---create-the-imagecapture-class"></a><span data-ttu-id="7d1c5-395">章 11-创建 ImageCapture 类</span><span class="sxs-lookup"><span data-stu-id="7d1c5-395">Chapter 11 - Create the ImageCapture class</span></span>

<span data-ttu-id="7d1c5-396">要创建的下一步类是*ImageCapture*类。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-396">The next class you are going to create is the *ImageCapture* class.</span></span>

<span data-ttu-id="7d1c5-397">此类负责：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-397">This class is responsible for:</span></span>

-   <span data-ttu-id="7d1c5-398">捕获映像使用 HoloLens 相机并将其存储在*应用*文件夹。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-398">Capturing an image using the HoloLens camera and storing it in the *App* Folder.</span></span>

-   <span data-ttu-id="7d1c5-399">处理来自用户的点击手势。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-399">Handling Tap gestures from the user.</span></span>

-   <span data-ttu-id="7d1c5-400">维护*Enum*值，该值确定是否运行该应用程序*Analysis*模式或*培训*模式。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-400">Maintaining the *Enum* value that determines if the application will run in *Analysis* mode or *Training* Mode.</span></span>

<span data-ttu-id="7d1c5-401">若要创建此类：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-401">To create this class:</span></span>

1.  <span data-ttu-id="7d1c5-402">转到**脚本**之前创建的文件夹。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-402">Go to the **Scripts** folder you created previously.</span></span>

2.  <span data-ttu-id="7d1c5-403">在文件夹中，右键单击，然后单击**创建 > C\#脚本**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-403">Right-click inside the folder, then click **Create > C\# Script**.</span></span> <span data-ttu-id="7d1c5-404">脚本命名为*ImageCapture*。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-404">Name the script *ImageCapture*.</span></span>

3.  <span data-ttu-id="7d1c5-405">双击新**ImageCapture**脚本以将其与打开**Visual Studio**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-405">Double-click on the new **ImageCapture** script to open it with **Visual Studio**.</span></span>

4.  <span data-ttu-id="7d1c5-406">使用以下内容替换该文件顶部的命名空间：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-406">Replace the namespaces at the top of the file with the following:</span></span>
...
5.  <span data-ttu-id="7d1c5-407">然后添加以下变量内的*ImageCapture*类，更高版本**start （)** 方法：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-407">Then add the following variables inside the *ImageCapture* class, above the **Start()** method:</span></span>
...
6.  <span data-ttu-id="7d1c5-408">为代码**Awake()** 并**start （)** 方法现在需要添加：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-408">Code for **Awake()** and **Start()** methods now needs to be added:</span></span>
...
7.  <span data-ttu-id="7d1c5-409">实现的点击动作时将调用的处理程序。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-409">Implement a handler that will be called when a Tap gesture occurs.</span></span>
    ...
    > [!NOTE] 
    > <span data-ttu-id="7d1c5-410">在中*分析*模式下， **TapHandler**方法充当一个开关，用于启动或停止照片捕获循环。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-410">In *Analysis* mode, the **TapHandler** method acts as a switch to start or stop the photo capture loop.</span></span>
    >
    > <span data-ttu-id="7d1c5-411">在中*培训*模式下，它将从捕获映像照相机。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-411">In *Training* mode, it will capture an image from the camera.</span></span>
    >
    > <span data-ttu-id="7d1c5-412">当光标位于绿色时，这意味着照相机是可用于获取映像。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-412">When the cursor is green, it means the camera is available to take the image.</span></span>
    >
    > <span data-ttu-id="7d1c5-413">当光标位于红色时，这意味着照相机正忙。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-413">When the cursor is red, it means the camera is busy.</span></span>

8.  <span data-ttu-id="7d1c5-414">添加应用程序使用启动映像捕获过程并将图像存储的方法。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-414">Add the method that the application uses to start the image capture process and store the image.</span></span>
...
9.  <span data-ttu-id="7d1c5-415">添加捕获照片和时准备好对其进行分析将调用处理程序。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-415">Add the handlers that will be called when the photo has been captured and for when it is ready to be analyzed.</span></span> <span data-ttu-id="7d1c5-416">然后将结果传递给*CustomVisionAnalyser*或*CustomVisionTrainer*具体取决于哪种模式上设置的代码。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-416">The result is then passed to the *CustomVisionAnalyser* or the *CustomVisionTrainer* depending on which mode the code is set on.</span></span>
...
10. <span data-ttu-id="7d1c5-417">请务必保存中所做的更改**Visual Studio**才会回到**Unity**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-417">Be sure to save your changes in **Visual Studio** before returning to **Unity**.</span></span>

11. <span data-ttu-id="7d1c5-418">现在，所有脚本都完成后，返回在 Unity 编辑器中，然后单击并拖动**SceneOrganiser**类派生**脚本**文件夹**Main Camera**对象中*层次结构面板*。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-418">Now that all the scripts have been completed, go back in the Unity Editor, then click and drag the **SceneOrganiser** class from the **Scripts** folder to the **Main Camera** object in the *Hierarchy Panel*.</span></span>

## <a name="chapter-12---before-building"></a><span data-ttu-id="7d1c5-419">第 12 章 — 生成前</span><span class="sxs-lookup"><span data-stu-id="7d1c5-419">Chapter 12 - Before building</span></span>

<span data-ttu-id="7d1c5-420">若要执行全面测试您的应用程序将需要旁加载它到在 HoloLens 上。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-420">To perform a thorough test of your application you will need to sideload it onto your HoloLens.</span></span>

<span data-ttu-id="7d1c5-421">执行操作之前，请确保：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-421">Before you do, ensure that:</span></span>

- <span data-ttu-id="7d1c5-422">中所述的所有设置[第 2 章](#chapter-2---training-your-custom-vision-project)正确设置。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-422">All the settings mentioned in the [Chapter 2](#chapter-2---training-your-custom-vision-project) are set correctly.</span></span>

- <span data-ttu-id="7d1c5-423">中的所有字段**Main Camera**，检查器面板中，正确分配。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-423">All the fields in the **Main Camera**, Inspector Panel, are assigned properly.</span></span>

- <span data-ttu-id="7d1c5-424">该脚本**SceneOrganiser**附加到**Main Camera**对象。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-424">The script **SceneOrganiser** is attached to the **Main Camera** object.</span></span>

- <span data-ttu-id="7d1c5-425">请确保插入你**预测键**成**predictionKey**变量。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-425">Make sure you insert your **Prediction Key** into the **predictionKey** variable.</span></span>

- <span data-ttu-id="7d1c5-426">已插入您**预测终结点**成**predictionEndpoint**变量。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-426">You have inserted your **Prediction Endpoint** into the **predictionEndpoint** variable.</span></span>

- <span data-ttu-id="7d1c5-427">已插入您**培训键**成**trainingKey**变量的*CustomVisionTrainer*类。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-427">You have inserted your **Training Key** into the **trainingKey** variable, of the *CustomVisionTrainer* class.</span></span>

- <span data-ttu-id="7d1c5-428">已插入您**项目 ID**成**projectId**变量的*CustomVisionTrainer*类。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-428">You have inserted your **Project ID** into the **projectId** variable, of the *CustomVisionTrainer* class.</span></span>

## <a name="chapter-13---build-and-sideload-your-application"></a><span data-ttu-id="7d1c5-429">第 13 章 — 生成和旁加载应用程序</span><span class="sxs-lookup"><span data-stu-id="7d1c5-429">Chapter 13 - Build and sideload your application</span></span>

<span data-ttu-id="7d1c5-430">若要开始*生成*过程：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-430">To begin the *Build* process:</span></span>

1.  <span data-ttu-id="7d1c5-431">转到**文件 > 生成设置**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-431">Go to **File > Build Settings**.</span></span>

2.  <span data-ttu-id="7d1c5-432">刻度线**Unity C\#项目**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-432">Tick **Unity C\# Projects**.</span></span>

3.  <span data-ttu-id="7d1c5-433">单击“生成” 。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-433">Click **Build**.</span></span> <span data-ttu-id="7d1c5-434">将启动 unity**文件资源管理器**窗口中，需要创建，然后选择要生成该应用程序的文件夹。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-434">Unity will launch a **File Explorer** window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="7d1c5-435">现在，创建该文件夹并将其命名**应用**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-435">Create that folder now, and name it **App**.</span></span> <span data-ttu-id="7d1c5-436">然后使用**应用程序**文件夹选择，单击**选择文件夹**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-436">Then with the **App** folder selected, click on **Select Folder**.</span></span>

4.  <span data-ttu-id="7d1c5-437">Unity 将开始构建您的项目**应用**文件夹。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-437">Unity will begin building your project to the **App** folder.</span></span>

5.  <span data-ttu-id="7d1c5-438">一次 Unity 已完成的生成 （它可能需要一些时间），它将打开**文件资源管理器**窗口在生成的位置 （检查任务栏中，因为它可能不总是会显示您的窗口上方，但会通知你添加了一个新窗口）。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-438">Once Unity has finished building (it might take some time), it will open a **File Explorer** window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

<span data-ttu-id="7d1c5-439">若要部署在 HoloLens 上：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-439">To deploy on HoloLens:</span></span>

1.  <span data-ttu-id="7d1c5-440">你将 （适用于远程部署），需要在 HoloLens IP 地址，以确保你 HoloLens 处于**开发人员模式**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-440">You will need the IP Address of your HoloLens (for Remote Deploy), and to ensure your HoloLens is in **Developer Mode**.</span></span> <span data-ttu-id="7d1c5-441">要实现此目的，请执行以下操作：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-441">To do this:</span></span>

    1.  <span data-ttu-id="7d1c5-442">戴上您 HoloLens，同时打开**设置**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-442">Whilst wearing your HoloLens, open the **Settings**.</span></span>

    2.  <span data-ttu-id="7d1c5-443">转到**网络和 Internet** > **的 Wi-fi** > **高级选项**</span><span class="sxs-lookup"><span data-stu-id="7d1c5-443">Go to **Network & Internet** > **Wi-Fi** > **Advanced Options**</span></span>

    3.  <span data-ttu-id="7d1c5-444">请注意**IPv4**地址。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-444">Note the **IPv4** address.</span></span>

    4.  <span data-ttu-id="7d1c5-445">接下来，导航回到**设置**，然后**更新和安全** > **面向开发人员**</span><span class="sxs-lookup"><span data-stu-id="7d1c5-445">Next, navigate back to **Settings**, and then to **Update & Security** > **For Developers**</span></span>

    5.  <span data-ttu-id="7d1c5-446">设置**上的开发人员模式**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-446">Set **Developer Mode On**.</span></span>

2.  <span data-ttu-id="7d1c5-447">导航到新的 Unity 生成 (**应用程序**文件夹)，然后打开解决方案文件与**Visual Studio**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-447">Navigate to your new Unity build (the **App** folder) and open the solution file with **Visual Studio**.</span></span>

3.  <span data-ttu-id="7d1c5-448">在中*解决方案配置*选择**调试**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-448">In the *Solution Configuration* select **Debug**.</span></span>

4.  <span data-ttu-id="7d1c5-449">在中*解决方案平台*，选择**x86，远程计算机**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-449">In the *Solution Platform*, select **x86, Remote Machine**.</span></span> <span data-ttu-id="7d1c5-450">系统会提示要插入**IP 地址**的远程设备 (HoloLens，在这种情况下，记下)。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-450">You will be prompted to insert the **IP address** of a remote device (the HoloLens, in this case, which you noted).</span></span>

    ![](images/AzureLabs-Lab302b-34.png)

5. <span data-ttu-id="7d1c5-451">转到**构建**菜单，并单击**部署解决方案**旁加载到你 HoloLens 应用程序。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-451">Go to **Build** menu and click on **Deploy Solution** to sideload the application to your HoloLens.</span></span>

6. <span data-ttu-id="7d1c5-452">在准备好启动在 HoloLens 上安装的应用列表中，现在应显示你的应用 ！</span><span class="sxs-lookup"><span data-stu-id="7d1c5-452">Your app should now appear in the list of installed apps on your HoloLens, ready to be launched!</span></span>

> [!NOTE]
> <span data-ttu-id="7d1c5-453">若要将部署到沉浸式头戴式耳机，设置**解决方案平台**到*本地计算机*，并设置**配置**到*调试*，与*x86*作为**平台**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-453">To deploy to immersive headset, set the **Solution Platform** to *Local Machine*, and set the **Configuration** to *Debug*, with *x86* as the **Platform**.</span></span> <span data-ttu-id="7d1c5-454">然后将其部署到本地计算机，使用**构建**菜单项，选择*部署解决方案*。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-454">Then deploy to the local machine, using the **Build** menu item, selecting *Deploy Solution*.</span></span> 

## <a name="to-use-the-application"></a><span data-ttu-id="7d1c5-455">若要使用应用程序：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-455">To use the application:</span></span>

<span data-ttu-id="7d1c5-456">若要切换之间的应用功能*培训*模式和*预测*模式需要更新**AppMode**变量，它位于**Awake()** 方法对位于*ImageCapture*类。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-456">To switch the app functionality between *Training* mode and *Prediction* mode you need to update the **AppMode** variable, located in the **Awake()** method located within the *ImageCapture* class.</span></span>

```
        // Change this flag to switch between Analysis mode and Training mode 
        AppMode = AppModes.Training;
```
<span data-ttu-id="7d1c5-457">或</span><span class="sxs-lookup"><span data-stu-id="7d1c5-457">or</span></span>
```
        // Change this flag to switch between Analysis mode and Training mode 
        AppMode = AppModes.Analysis;
```

<span data-ttu-id="7d1c5-458">在中*培训*模式：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-458">In *Training* mode:</span></span>

- <span data-ttu-id="7d1c5-459">看看**鼠标**或**键盘**，并使用**点击手势**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-459">Look at **Mouse** or **Keyboard** and use the **Tap gesture**.</span></span>

- <span data-ttu-id="7d1c5-460">接下来，文本将显示要求你提供一个标记。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-460">Next, text will appear asking you to provide a tag.</span></span>

- <span data-ttu-id="7d1c5-461">指示**鼠标**或**键盘**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-461">Say either **Mouse** or **Keyboard**.</span></span>


<span data-ttu-id="7d1c5-462">在中*预测*模式：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-462">In *Prediction* mode:</span></span>

- <span data-ttu-id="7d1c5-463">查看对象，并使用**点击手势**。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-463">Look at an object and use the **Tap gesture**.</span></span>

- <span data-ttu-id="7d1c5-464">文本将显示提供检测到，（这规范化的） 的概率最高的对象。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-464">Text will appear providing the object detected, with the highest probability (this is normalized).</span></span>

## <a name="chapter-14---evaluate-and-improve-your-custom-vision-model"></a><span data-ttu-id="7d1c5-465">14-评估和改进您的自定义影像模型</span><span class="sxs-lookup"><span data-stu-id="7d1c5-465">Chapter 14 - Evaluate and improve your Custom Vision model</span></span>

<span data-ttu-id="7d1c5-466">若要使你的服务更准确，您需要继续训练用于预测的模型。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-466">To make your Service more accurate, you will need to continue to train the model used for prediction.</span></span> <span data-ttu-id="7d1c5-467">这通过使用两个新应用程序，来实现*培训*并*预测*模式下，使用后一种无需访问门户，它是这一章中涵盖的内容。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-467">This is accomplished through using your new application, with both the *training* and *prediction* modes, with the latter requiring you to visit the portal, which is what is covered in this Chapter.</span></span> <span data-ttu-id="7d1c5-468">要准备好，若要再次访问你的门户很多时候，可以不断改进您的模型。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-468">Be prepared to revisit your portal many times, to continually improve your model.</span></span>

1. <span data-ttu-id="7d1c5-469">同样，转到你的 Azure 自定义视觉门户并在项目中后，选择*预测*（从页面的顶部中心） 的选项卡：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-469">Head to your Azure Custom Vision Portal again, and once you are in your project, select the *Predictions* tab (from the top center of the page):</span></span>

    ![](images/AzureLabs-Lab302b-35.png)

2. <span data-ttu-id="7d1c5-470">你将看到所有已发送到你的服务，同时你的应用程序正在运行的映像。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-470">You will see all the images which were sent to your Service whilst your application was running.</span></span> <span data-ttu-id="7d1c5-471">如果将鼠标悬停鼠标悬停在图像，他们将您提供有关该映像所做的预测：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-471">If you hover over the images, they will provide you with the predictions that were made for that image:</span></span>

    ![](images/AzureLabs-Lab302b-36.png)

3. <span data-ttu-id="7d1c5-472">选择其中一个映像以将其打开。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-472">Select one of your images to open it.</span></span> <span data-ttu-id="7d1c5-473">一旦打开，你将看到右侧进行该图像的预测。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-473">Once open, you will see the predictions made for that image to the right.</span></span> <span data-ttu-id="7d1c5-474">如果您预测正确的并且你想要将此映像添加到你的服务的训练模型中，单击*我标记*输入框，然后选择你想要将相关联的标记。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-474">If you the predictions were correct, and you wish to add this image to your Service's training model, click the *My Tags* input box, and select the tag you wish to associate.</span></span> <span data-ttu-id="7d1c5-475">完成后，单击*保存并关闭*靠下右对齐，向按钮，然后继续学习下一幅图像。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-475">When you are finished, click the *Save and close* button to the bottom right, and continue on to the next image.</span></span>

    ![](images/AzureLabs-Lab302b-37.png)

4. <span data-ttu-id="7d1c5-476">返回到网格的映像后，你会注意到已添加到标记 （并保存），映像将被删除。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-476">Once you are back to the grid of images, you will notice the images which you have added tags to (and saved), will be removed.</span></span> <span data-ttu-id="7d1c5-477">如果您发现您认为不具有在其中你带标记的项的任何映像，则可以删除它们，通过单击该映像 （可以执行此操作的多个映像） 上的刻度线，然后单击*删除*网格页右上角。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-477">If you find any images which you think do not have your tagged item within them, you can delete them, by clicking the tick on that image (can do this for several images) and then clicking *Delete* in the upper right corner of the grid page.</span></span> <span data-ttu-id="7d1c5-478">在弹出窗口后面，你可以单击*可以，请删除*或*否*，以确认删除，或分别取消。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-478">On the popup that follows, you can click either *Yes, delete* or *No*, to confirm the deletion or cancel it, respectively.</span></span> 

    ![](images/AzureLabs-Lab302b-38.png)

5. <span data-ttu-id="7d1c5-479">若要继续，准备就绪后，单击绿色*训练*在右上角的按钮。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-479">When you are ready to proceed, click the green *Train* button in the top right.</span></span> <span data-ttu-id="7d1c5-480">你的服务模型将使用的所有映像现在已提供 （这将使其更准确） 训练。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-480">Your Service model will be trained with all the images which you have now provided (which will make it more accurate).</span></span> <span data-ttu-id="7d1c5-481">培训完成后，请务必单击*设为默认*次，按钮，以便你*预测 URL*仍会继续使用你的服务的最新迭代。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-481">Once the training is complete, make sure to click the *Make default* button once more, so that your *Prediction URL* continues to use the most up-to-date iteration of your Service.</span></span>

    <span data-ttu-id="7d1c5-482">![](images/AzureLabs-Lab302b-39.png) ![](images/AzureLabs-Lab302b-40.png)</span><span class="sxs-lookup"><span data-stu-id="7d1c5-482">![](images/AzureLabs-Lab302b-39.png) ![](images/AzureLabs-Lab302b-40.png)</span></span>

## <a name="your-finished-custom-vision-api-application"></a><span data-ttu-id="7d1c5-483">完成自定义视觉 API 的应用程序</span><span class="sxs-lookup"><span data-stu-id="7d1c5-483">Your finished Custom Vision API application</span></span>

<span data-ttu-id="7d1c5-484">祝贺你，构建利用 Azure 自定义视觉 API 能够识别现实世界对象、 训练服务模型，并显示的内容出现的置信度的混合的现实应用。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-484">Congratulations, you built a mixed reality app that leverages the Azure Custom Vision API to recognize real world objects, train the Service model, and display confidence of what has been seen.</span></span>

![](images/AzureLabs-Lab302b-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="7d1c5-485">Bonus 练习</span><span class="sxs-lookup"><span data-stu-id="7d1c5-485">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="7d1c5-486">练习 1</span><span class="sxs-lookup"><span data-stu-id="7d1c5-486">Exercise 1</span></span>

<span data-ttu-id="7d1c5-487">训练你**自定义影像服务**识别更多的对象。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-487">Train your **Custom Vision Service** to recognize more objects.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="7d1c5-488">练习 2</span><span class="sxs-lookup"><span data-stu-id="7d1c5-488">Exercise 2</span></span>

<span data-ttu-id="7d1c5-489">作为一种方式扩展您学到了什么，完成以下练习：</span><span class="sxs-lookup"><span data-stu-id="7d1c5-489">As a way to expand on what you have learned, complete the following exercises:</span></span>

<span data-ttu-id="7d1c5-490">识别对象时播放声音。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-490">Play a sound when an object is recognized.</span></span>

### <a name="exercise-3"></a><span data-ttu-id="7d1c5-491">练习 3</span><span class="sxs-lookup"><span data-stu-id="7d1c5-491">Exercise 3</span></span>

<span data-ttu-id="7d1c5-492">若要重新训练你的服务使用相同的图像分析您的应用程序，因此若要使服务更准确地使用 API （执行预测和同时培训）。</span><span class="sxs-lookup"><span data-stu-id="7d1c5-492">Use the API to re-train your Service with the same images your app is analyzing, so to make the Service more accurate (do both prediction and training simultaneously).</span></span>
