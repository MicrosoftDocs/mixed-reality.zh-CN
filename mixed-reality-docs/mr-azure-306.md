...
## <a name="chapter-1---the-azure-portal-creating-the-azure-storage-account"></a><span data-ttu-id="f9fbe-146">第1章-Azure 门户: 创建 Azure 存储帐户</span><span class="sxs-lookup"><span data-stu-id="f9fbe-146">Chapter 1 - The Azure Portal: creating the Azure Storage Account</span></span>
...
4.  <span data-ttu-id="f9fbe-155">在 "**创建存储帐户**" 选项卡中:</span><span class="sxs-lookup"><span data-stu-id="f9fbe-155">In the **Create storage account** tab:</span></span>
    ...
    7.  <span data-ttu-id="f9fbe-162">选择**订阅**。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-162">Select a **Subscription**.</span></span>
...
13. <span data-ttu-id="f9fbe-212">你会注意到你的视频已成功上传。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-212">You will notice that your video has been successfully uploaded.</span></span> <span data-ttu-id="f9fbe-213">单击它。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-213">Click on it.</span></span>
...
<span data-ttu-id="f9fbe-269">下面是使用混合现实进行开发的典型设置, 因此, 这是其他项目的一个不错的模板。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-269">The following is a typical set up for developing with the Mixed Reality, and as such, is a good template for other projects.</span></span>
...
5.  <span data-ttu-id="f9fbe-283">另外, 请确保:</span><span class="sxs-lookup"><span data-stu-id="f9fbe-283">Also make sure that:</span></span>
    ...
    6.  <span data-ttu-id="f9fbe-289">请不要担心现在设置**场景**, 因为稍后会设置它们。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-289">Do not worry about setting up **Scenes** right now, as you will set these up later.</span></span>
...
## <a name="chapter-4---importing-the-insideoutsphere-unity-package"></a><span data-ttu-id="f9fbe-306">第4章-导入 InsideOutSphere Unity 包</span><span class="sxs-lookup"><span data-stu-id="f9fbe-306">Chapter 4 - Importing the InsideOutSphere Unity package</span></span>
...
9.  <span data-ttu-id="f9fbe-389">在**start ()** 方法之后, 插入**PlayVideo ()** *IEnumerator*方法, 该方法将用于无缝启动视频 (因此不会出现断断续续)。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-389">Following the **Start()** method, insert the **PlayVideo()** *IEnumerator* method, which will be used to start videos seamlessly (so no stutter is seen).</span></span>
...
14. <span data-ttu-id="f9fbe-399">将**InsideOutSphere**对象从 "**层次结构" 面板**拖动到**球体**槽, 如下图所示。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-399">Drag the **InsideOutSphere** object from the **Hierarchy Panel** to the **Sphere** slot, as shown in the image below.</span></span>
...
4.  <span data-ttu-id="f9fbe-425">在 "文件名 **:** 文本" 字段中, 键入**VideoScene1**, 然后按 "**保存**"。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-425">In the **File name:** text field, type **VideoScene1**, then press **Save**.</span></span>
...
14. <span data-ttu-id="f9fbe-461">现在, **GazeButton**已部分更新, 但要看起来不同, 但现在你将创建新的**材料**, 使其看起来完全不同, 并且更容易识别为不同于第一个场景中的对象的对象。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-461">The **GazeButton** is now partially updated, to look different, however, you will now create a new **Material**, so that it looks completely different, and is easier to recognize as a different object, than the object in the first scene.</span></span>

15. <span data-ttu-id="f9fbe-462">导航到 "**项目" 面板**中的 "**材料**" 文件夹。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-462">Navigate to your **Materials** folder, within the **Project Panel**.</span></span> <span data-ttu-id="f9fbe-463">复制**ButtonMaterial**材料 (按键盘上的**Ctrl** + **D** , 或左键单击**材料**, 然后从 "**编辑**文件" 菜单选项中选择 "**复制**")。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-463">Duplicate the **ButtonMaterial** Material (press **Ctrl** + **D** on the keyboard, or left-click the **Material**, then from the **Edit** file menu option, select **Duplicate**).</span></span>

    <span data-ttu-id="f9fbe-464">![第7章-设置两个 unity 场景](images/AzureLabs-Lab6-55.png)
    ![第7章-设置两个 unity 场景](images/AzureLabs-Lab6-56.png)</span><span class="sxs-lookup"><span data-stu-id="f9fbe-464">![Chapter 7 -- Setup the two Unity Scenes](images/AzureLabs-Lab6-55.png)
![Chapter 7 -- Setup the two Unity Scenes](images/AzureLabs-Lab6-56.png)</span></span>

16. <span data-ttu-id="f9fbe-465">选择新的**ButtonMaterial**材料 (此处名为**ButtonMaterial 1**), 并在**检查器**中单击 " **Albedo**色" 窗口。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-465">Select the new **ButtonMaterial** Material (here named **ButtonMaterial 1**), and within the **Inspector**, click the **Albedo** color window.</span></span> <span data-ttu-id="f9fbe-466">此时将显示一个弹出窗口, 您可以在其中选择其他颜色 (选择任何喜欢的颜色), 然后关闭弹出窗口。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-466">A popup will appear, where you can select another color (choose whichever you like), then close the popup.</span></span> <span data-ttu-id="f9fbe-467">材料将是其自身的实例, 并且不同于原始。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-467">The Material will be its own instance, and different to the original.</span></span>

    ![第7章--设置两个 Unity 场景](images/AzureLabs-Lab6-57.png)

17. <span data-ttu-id="f9fbe-469">将新**材料**拖到**GazeButton**子级上, 立即完全更新其外观, 使其与第一个场景按钮容易区分。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-469">Drag the new **Material** onto the **GazeButton** child, to now completely update its look, so that it is easily distinguishable from the first scenes button.</span></span>

    ![第7章--设置两个 Unity 场景](images/AzureLabs-Lab6-58.png)

18. <span data-ttu-id="f9fbe-471">此时, 可以在生成 UWP 项目之前在编辑器中测试项目。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-471">At this point you can test the project in the Editor before building the UWP project.</span></span>

    -  <span data-ttu-id="f9fbe-472">按下**编辑器**中的 "**播放**" 按钮, 戴上耳机。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-472">Press the **Play** button in the **Editor** and wear your headset.</span></span>

        ![第7章--设置两个 Unity 场景](images/AzureLabs-Lab6-59.png)

19. <span data-ttu-id="f9fbe-474">查看两个**GazeButton**对象, 以便在第一个和第二个视频之间切换。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-474">Look at the two **GazeButton** objects to switch between the first and second video.</span></span>

## <a name="chapter-8---build-the-uwp-solution"></a><span data-ttu-id="f9fbe-475">第8章-构建 UWP 解决方案</span><span class="sxs-lookup"><span data-stu-id="f9fbe-475">Chapter 8 - Build the UWP Solution</span></span>

<span data-ttu-id="f9fbe-476">确保编辑器没有错误之后, 就可以开始生成了。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-476">Once you have ensured that the editor has no errors, you are ready to Build.</span></span>

<span data-ttu-id="f9fbe-477">若要生成:</span><span class="sxs-lookup"><span data-stu-id="f9fbe-477">To Build:</span></span>

1.  <span data-ttu-id="f9fbe-478">单击 "**文件" > "保存**", 保存当前场景。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-478">Save the current scene by clicking on **File > Save**.</span></span>

2.  <span data-ttu-id="f9fbe-479">选中 "名为**Unity C\#项目**的" 框 (这一点很重要, 因为它将允许您在生成完成后编辑类)。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-479">Check the box called **Unity C\# Projects** (this is important because it will allow you to edit the classes after build is completed).</span></span>

3.  <span data-ttu-id="f9fbe-480">请参阅**文件 > 生成设置**, 单击 "**生成**"。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-480">Go to **File > Build Settings**, click on **Build**.</span></span>

4.  <span data-ttu-id="f9fbe-481">系统将提示你选择要 buildthe 解决方案的文件夹。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-481">You will be prompted to select the folder where you want to buildthe Solution.</span></span>

5.  <span data-ttu-id="f9fbe-482">创建一个**生成**文件夹, 在该文件夹中创建另一个文件夹, 其中包含所选的适当名称。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-482">Create a **BUILDS** folder and within that folder create another folder with an appropriate name of your choice.</span></span>

6.  <span data-ttu-id="f9fbe-483">单击新文件夹, 然后单击 "**选择文件夹**", 然后选择该文件夹以在该位置开始生成。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-483">Click your new folder and then click **Select Folder**, so to choose that folder, to begin the build at that location.</span></span>

    <span data-ttu-id="f9fbe-484">![第8章-构建 uwp 解决方案](images/AzureLabs-Lab6-60.png)
    ![第8章-构建 uwp 解决方案](images/AzureLabs-Lab6-61.png)</span><span class="sxs-lookup"><span data-stu-id="f9fbe-484">![Chapter 8 -- Build the UWP Solution](images/AzureLabs-Lab6-60.png)
![Chapter 8 -- Build the UWP Solution](images/AzureLabs-Lab6-61.png)</span></span>

7.  <span data-ttu-id="f9fbe-485">Unity 完成生成后 (可能需要一些时间), 将在生成的位置打开**文件资源管理器**窗口。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-485">Once Unity has finished building (it might take some time), it will open a **File Explorer** window at the location of your build.</span></span>

## <a name="chapter-9---deploy-on-local-machine"></a><span data-ttu-id="f9fbe-486">第9章-在本地计算机上部署</span><span class="sxs-lookup"><span data-stu-id="f9fbe-486">Chapter 9 - Deploy on Local Machine</span></span>

<span data-ttu-id="f9fbe-487">完成生成后, "**文件资源管理器**" 窗口将出现在生成的位置。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-487">Once the build has been completed, a **File Explorer** window will appear at the location of your build.</span></span> <span data-ttu-id="f9fbe-488">打开名为的文件夹, 然后双击该文件夹中的解决方案 (.sln) 文件, 以通过 Visual Studio 2017 打开解决方案。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-488">Open the Folder you named and built to, then double click on the solution (.sln) file within that folder, to open your solution with Visual Studio 2017.</span></span>

<span data-ttu-id="f9fbe-489">唯一要做的事情就是将您的应用程序部署到您的计算机 (或*本地计算机*)。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-489">The only thing left to do is deploy your app to your computer (or *Local Machine*).</span></span>

<span data-ttu-id="f9fbe-490">部署到本地计算机:</span><span class="sxs-lookup"><span data-stu-id="f9fbe-490">To deploy to Local Machine:</span></span>

1.  <span data-ttu-id="f9fbe-491">在**Visual Studio 2017**中, 打开刚刚创建的解决方案文件。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-491">In **Visual Studio 2017**, open the solution file that has just been created.</span></span>

2.  <span data-ttu-id="f9fbe-492">在**解决方案平台**中, 选择 " **X86, 本地计算机**"。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-492">In the **Solution Platform**, select **x86, Local Machine**.</span></span>

3.  <span data-ttu-id="f9fbe-493">在**解决方案配置**中, 选择 "**调试**"。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-493">In the **Solution Configuration** select **Debug**.</span></span>

    ![第9章--在本地计算机上部署](images/AzureLabs-Lab6-62.png)

4.  <span data-ttu-id="f9fbe-495">你现在需要将任何包还原到你的解决方案中。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-495">You will now need to restore any packages to your solution.</span></span> <span data-ttu-id="f9fbe-496">右键单击**解决方案**, 然后单击 "**还原解决方案的 NuGet 包 ...** "。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-496">Right-click on your **Solution**, and click **Restore NuGet Packages for Solution...**</span></span>

    > [!NOTE] 
    > <span data-ttu-id="f9fbe-497">完成此操作的原因是, 需要以 Unity 构建的目标来处理本地计算机引用。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-497">This is done because the packages which Unity built need to be targeted to work with your local machines references.</span></span>

5.  <span data-ttu-id="f9fbe-498">中转到 "**生成" 菜单**, 然后单击 "**部署解决方案**", 将应用程序旁加载到计算机上。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-498">Go to **Build menu** and click on **Deploy Solution** to sideload the application to your machine.</span></span> <span data-ttu-id="f9fbe-499">Visual Studio 将首先生成并部署你的应用程序。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-499">Visual Studio will first build and then deploy your application.</span></span>

6.  <span data-ttu-id="f9fbe-500">应用现在应显示在已安装的应用列表中, 可以启动。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-500">Your App should now appear in the list of installed apps, ready to be launched.</span></span>

    ![第9章--在本地计算机上部署](images/AzureLabs-Lab6-63.png)

<span data-ttu-id="f9fbe-502">运行混合现实应用程序时, 会在应用中使用的**InsideOutSphere**模型中。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-502">When you run the Mixed Reality application, you will you be within the **InsideOutSphere** model which you used within your app.</span></span> <span data-ttu-id="f9fbe-503">此球体会将视频流式传输到该位置, 从而提供一种360度的传入视频 (这是 filmed 的一种形式)。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-503">This sphere will be where the video will be streamed to, providing a 360-degree view, of the incoming video (which was filmed for this kind of perspective).</span></span> <span data-ttu-id="f9fbe-504">如果需要几秒钟时间来加载视频, 请不要惊讶, 你的应用程序将受到你的可用 Internet 速度的限制, 因为需要获取并下载视频, 以便流式传输到你的应用程序。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-504">Do not be surprised if the video takes a couple of seconds to load, your app is subject to your available Internet speed, as the video needs to be fetched and then downloaded, so to stream into your app.</span></span>
<span data-ttu-id="f9fbe-505">准备就绪后, 请通过 gazing 在红色球上来更改场景并打开第二个视频!</span><span class="sxs-lookup"><span data-stu-id="f9fbe-505">When you are ready, change scenes and open your second video, by gazing at the red sphere!</span></span> <span data-ttu-id="f9fbe-506">然后, 在第二个场景中使用蓝色立方体即可自由返回!</span><span class="sxs-lookup"><span data-stu-id="f9fbe-506">Then feel free to go back, using the blue cube in the second scene!</span></span>

## <a name="your-finished-azure-media-service-application"></a><span data-ttu-id="f9fbe-507">已完成的 Azure 媒体服务应用程序</span><span class="sxs-lookup"><span data-stu-id="f9fbe-507">Your finished Azure Media Service application</span></span>
 
<span data-ttu-id="f9fbe-508">恭喜, 你构建了一个利用 Azure 媒体服务流式传输360视频的混合现实应用。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-508">Congratulations, you built a mixed reality app that leverages the Azure Media Service to stream 360 videos.</span></span>

![实验室结果](images/AzureLabs-Lab6-00.png)

![实验室结果](images/AzureLabs-Lab6-01.png)

## <a name="bonus-exercises"></a><span data-ttu-id="f9fbe-511">额外练习</span><span class="sxs-lookup"><span data-stu-id="f9fbe-511">Bonus Exercises</span></span>

<span data-ttu-id="f9fbe-512">**练习1**</span><span class="sxs-lookup"><span data-stu-id="f9fbe-512">**Exercise 1**</span></span>

<span data-ttu-id="f9fbe-513">在本教程中, 完全可以仅使用一个场景来更改视频。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-513">It is entirely possible to only use a single scene to change videos within this tutorial.</span></span> <span data-ttu-id="f9fbe-514">试验应用程序并将其放入一个场景!</span><span class="sxs-lookup"><span data-stu-id="f9fbe-514">Experiment with your application and make it into a single scene!</span></span> <span data-ttu-id="f9fbe-515">甚至可能将其他视频添加到组合。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-515">Perhaps even add another video to the mix.</span></span>

<span data-ttu-id="f9fbe-516">**练习2**</span><span class="sxs-lookup"><span data-stu-id="f9fbe-516">**Exercise 2**</span></span>

<span data-ttu-id="f9fbe-517">试验 Azure 和 Unity, 尝试实现此功能, 使应用程序能够根据 Internet 连接的强度, 自动选择不同文件大小的视频。</span><span class="sxs-lookup"><span data-stu-id="f9fbe-517">Experiment with Azure and Unity, and attempt to implement the ability for the app to automatically select a video with a different file size, depending on the strength of an Internet connection.</span></span>
