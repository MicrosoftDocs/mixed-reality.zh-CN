...
为了使 Windows Mixed Reality 平台来优化全息图稳定性，它依赖于要准确且匹配在屏幕上的任何呈现的全息的深度缓冲区。 因此，使用共享上的深度缓冲区，很重要时呈现颜色，还呈现深度。 在 Unity 中，大多数的不透明或 TransparentCutout 材料将呈现深度，默认情况下，但透明，文本对象通常不会呈现深度虽然这是依赖着色器，等等。 

如果使用标准混合现实工具包着色器，呈现为透明对象的深度：
1) 选择使用标准 MRTK 着色器的透明材料，然后打开检查器编辑器窗口
2) 设置**呈现模式**到**自定义**然后设置**模式**到**透明**并将最后设置**深度编写**到**上**
