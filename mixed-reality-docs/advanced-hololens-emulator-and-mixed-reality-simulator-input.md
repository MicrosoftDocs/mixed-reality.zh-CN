...
[手势](gestures.md)和运动控制器输入与它们的物理设备密切对应:
* **操作**:这会模拟按食指捏的操作, 或在控制器上拉取操作按钮。 例如, 操作输入可用于模拟攻攻敲击手势, 滚动查看内容, 以及按下并保持。
* **[布隆](gestures.md#bloom)/System 手势或 Home**:使用 HoloLens 布隆/系统笔势或控制器的 "主页" 按钮可返回到 shell 并执行系统操作。

在 HoloLens 2 中, 手具有丰富的 reprresentation。  除了进行跟踪/未跟踪以及可用于驾驶手势的情况外, 我们现在有一个可供用户使用的有表述的主干模型, 并向开发人员公开。  这会在每个局上引入26个跟踪点。  
* **接点**:给定跟踪手的20个跟踪位置中的一个。 这会有一个与之关联的3d 空间。
* **姿势**:跟踪手中所有接头的完整集合。 目前, 这是一个26个联接的集合。 

目前, 我们不会通过仿真程序用户界面分别公开每个接点位置的直接控制, 但你可以通过模拟 API 来设置它们。 相反, 我们有一组有用的代表, 模拟器允许您在之间切换。

你还可以控制模拟传感器输入的状态:
* **重置**:这会将所有模拟传感器恢复为其默认值。  从 HoloLens 2 模拟器开始, 通过使用合适的修改键或按钮 (左和/或右 Alt, 或游戏板上的左和/或右缓冲器), 可以将重置范围限制为一方或两方。
* **跟踪**:循环遍历位置跟踪模式。 这包括：
  * **默认**：操作系统会根据系统发出的请求选择最佳跟踪模式。
...
|  俯仰 |  向上/向下箭头 |  向上/向下拖动 |  右操纵杆向上/向下 | 
|  Roll |  Q/E |  |  DPad 左/右 | 
|  X |  A/D |  |  左操纵杆向左/向右 | 
|  Y |  Page up/page down |  |  DPad | 
|  Z |  W/S |  |  向左/向下移动操纵杆 | 
|  Action |  Enter 或 space |  右按钮 |  按钮或触发器 | 
...
注意:控制器按钮可以已定位到一个手型/控制器或另一个使用手形目标修饰符。
...
| Object | 键盘修饰符 | 控制器修饰符 | 模拟器 UI 修饰符 |
|----------|----------|----------|----------|
| Body | （默认值） | （默认值） | （默认值） |
| 头 | 保持 H | (不可用) | (不可用) |
...
|  俯仰 |  开启/关闭头 |  上移/下移 |  开启/关闭头 | 眼睛查看/关闭 | 
