...
6.  您的表将在您的 Visual Studio 窗口的中心打开。 单击表图标，其中包含 **+** （加号）。
...
22. 文件上传后，它将显示在右侧的面板中。 单击它会在**函数**编辑器中打开它。 它必须与下一个图像**完全**相同（低于步骤23）。
...
> [!WARNING]
> *不要直接编辑通知中心名称*（应使用**高级编辑器**完成此操作，前提是你遵循了前面的步骤。
...
5.  如果仍在**文件** > **生成设置**，请确保：
    ...
    6.  在此过程中，需要保存场景，并将其添加到生成中。
        ...
        3. 打开新创建的**场景**文件夹，然后在 "文件名 **：** 文本" 字段中，键入**NH\_Desktop\_场景**，然后按 "**保存**"。
...
2.  在弹出的 "**导入 Unity 包**" 框中，可以选择 "*插件*" 下的所有内容 \> * Storage * * *。  取消选中所有其他内容，因为本课程不需要这样做。
...
1.  右键单击位于 "项目" 面板中的 "**资产**" 文件夹，**创建** > **文件夹**。 命名文件夹**脚本**。
...
3.  将弹出一个**文件资源管理器**窗口，提示你提供要生成的位置。 在左上角单击 "**新建文件夹**"，创建一个新文件夹，并将其命名为 "**生成**"。
    ...
    1.  打开 "新建**生成**" 文件夹，并创建另一个文件夹（使用 "**新建文件夹**"），并将其命名为 " **NH\_桌面\_应用**"。

        ![文件夹名称 NH_Desktop_App](images/AzureLabs-Lab8-78.png)

    2.  选择**NH\_桌面\_应用**。 单击 "**选择文件夹**"。 项目将需要一分钟或更长时间才能生成。
...
5.  请参阅**文件** > **生成设置**，并确保：
    ...
    6.  在此过程中，需要保存场景，并将其添加到生成中。
        ...
        3. 打开新创建的**场景**文件夹，然后在 "文件名 **：** 文本" 字段中，键入**NH\_MR\_场景**，然后按 "**保存**"。
...
1.  使用**资产** > **导入包** > **自定义包**"菜单选项，将从上面下载的 unitypackage 添加到 Unity。

2.  在弹出的 "**导入 Unity 包**" 框中，可以选择 > **存储**的**插件**下的所有内容。
...
1.  右键单击 "**脚本**" 文件夹中，单击 "**创建**， **C\# 脚本**"。 将脚本命名为**NotificationReceiver**。
    ...
    ![将其命名为 NotificationReceiver](images/AzureLabs-Lab8-96.png)
...
9.  每次收到通知时，都会触发以下处理程序**信道\_PushNotificationReceived （）** 。 它将反序列化通知，该通知将是已移动到桌面应用程序中的 Azure 表实体，然后将 MR 场景中的相应 GameObject 移到相同的位置。 
...
4.  将弹出一个**文件资源管理器**窗口，提示你提供要生成的位置。 在左上角单击 "**新建文件夹**"，创建一个新文件夹，并将其命名为 "**生成**"。
    ...
    1.  打开 "新建**生成**" 文件夹，并创建另一个文件夹（使用 "**新文件夹**"），并将其命名为 " **NH\_MR\_应用**"。

        ![创建 NH_MR_Apps 文件夹](images/AzureLabs-Lab8-101.png)

    2.  选择**NH\_MR\_应用**。 单击 "**选择文件夹**"。 项目将需要一分钟或更长时间才能生成。
