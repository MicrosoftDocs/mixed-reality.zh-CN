...
在语音输入处理程序（脚本）组件上，单击 "小 **+** " 按钮，将关键字元素添加到关键字列表中：

![mrlearning-base](images/mrlearning-base/tutorial5-section1-step5-1.png)

单击新创建的**元素 0**展开它，然后在 "**关键字**" 下拉列表中，选择前面创建的 "**播放音乐**" 关键字：
...
在 "层次结构" 窗口中，**创建四个多维数据集**作为**PanGesture**对象的子对象，并将其转换**比例**设置为 X = 0.15，Y = 0.15，Z = 0.15：

![mrlearning-base](images/mrlearning-base/tutorial5-section2-step5-1.png)

若要将多维数据集均匀地缩小，并保存一些时间，请将**网格对象集合（脚本）** 组件添加到多维数据集的父对象（即**PanGesture**对象），并按如下所示配置网格对象集合（脚本）：
