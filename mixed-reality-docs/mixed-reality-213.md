...
## <a name="topics-covered-in-this-tutorial"></a><span data-ttu-id="d5e9d-115">在本教程中涵盖的主题</span><span class="sxs-lookup"><span data-stu-id="d5e9d-115">Topics covered in this tutorial</span></span>
...
* <span data-ttu-id="d5e9d-158">在中**Inspector**面板中，向下滚动到底部</span><span class="sxs-lookup"><span data-stu-id="d5e9d-158">In the **Inspector** panel, scroll down to the bottom</span></span>
...
* <span data-ttu-id="d5e9d-198">了解如何呈现控制器模型在 Unity 的游戏模式下和在运行时的动作。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-198">Learn how to render motion controller models in Unity's game mode and at runtime.</span></span>
...
<span data-ttu-id="d5e9d-217">**MotionControllerVisualizer script**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-217">**MotionControllerVisualizer script**</span></span>
...
<span data-ttu-id="d5e9d-251">**说明**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-251">**Instructions**</span></span>
...
<span data-ttu-id="d5e9d-275">**将对象附加到的控制器的另一种方法**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-275">**Alternative method for attaching objects to the controllers**</span></span>
...
<span data-ttu-id="d5e9d-290">**显示/隐藏 UI 与 Unity 的动画控制器**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-290">**Showing/hiding UI with Unity's Animation controller**</span></span>
...
<span data-ttu-id="d5e9d-304">在**update （)** 根据**可见**属性，它将触发颜色选取器的动画器组件中的显示和隐藏动画触发器</span><span class="sxs-lookup"><span data-stu-id="d5e9d-304">In **Update()**, based on **visible** property, it triggers Show and Hide animation triggers in the color picker's animator component</span></span>
...
## <a name="chapter-4---overriding-controller-model"></a><span data-ttu-id="d5e9d-309">第 4 章-重写控制器模型</span><span class="sxs-lookup"><span data-stu-id="d5e9d-309">Chapter 4 - Overriding controller model</span></span>
...
## <a name="chapter-5---painting-with-select-input"></a><span data-ttu-id="d5e9d-329">第 5 章-使用选择的输入进行绘制</span><span class="sxs-lookup"><span data-stu-id="d5e9d-329">Chapter 5 - Painting with Select input</span></span>
...
<span data-ttu-id="d5e9d-363">有关**选择**事件，请在**SpawnObject()**，新对象是实例化、 无父级且已发布到世界。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-363">For a **Select** event, in **SpawnObject()**, a new object is instantiated, un-parented and released into the world.</span></span>
...
<span data-ttu-id="d5e9d-424">**BrushSelector 脚本**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-424">**BrushSelector script**</span></span>
...
<span data-ttu-id="d5e9d-441">而不是**InteractionSourcePressType.Select**中的事件**InteractionSourcePressed()**，就可以通过按下的量的模拟值**selectPressedAmount**.</span><span class="sxs-lookup"><span data-stu-id="d5e9d-441">Instead of the **InteractionSourcePressType.Select** event in the **InteractionSourcePressed()**, you can get the analog value of the pressed amount through **selectPressedAmount**.</span></span> <span data-ttu-id="d5e9d-442">此值可以在中检索**InteractionSourceUpdated()**。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-442">This value can be retrieved in **InteractionSourceUpdated()**.</span></span>
...
<span data-ttu-id="d5e9d-443">**橡皮擦脚本**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-443">**Eraser script**</span></span>

<span data-ttu-id="d5e9d-444">**橡皮擦**是一种特殊的重写基的画笔**画笔**的**DrawOverTime()** 函数。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-444">**Eraser** is a special type of brush that overrides the base **Brush**'s **DrawOverTime()** function.</span></span> <span data-ttu-id="d5e9d-445">绘制为真，橡皮擦将检查以确定其提示是否与任何现有的画笔笔画相交。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-445">While Draw is true, the eraser checks to see if its tip intersects with any existing brush strokes.</span></span> <span data-ttu-id="d5e9d-446">如果是这样，它们都添加到队列，向下要收缩和删除。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-446">If it does, they are added to a queue to be shrunk down and deleted.</span></span>

## <a name="advanced-design---teleportation-and-locomotion"></a><span data-ttu-id="d5e9d-447">高级的设计-Teleportation 和 locomotion</span><span class="sxs-lookup"><span data-stu-id="d5e9d-447">Advanced design - Teleportation and locomotion</span></span>

<span data-ttu-id="d5e9d-448">如果你想要允许用户与使用摇杆 teleportation 在场景周围移动，请使用**MixedRealityCameraParent**而不是**MixedRealityCamera**。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-448">If you want to allow the user to move around the scene with teleportation using thumbstick, use **MixedRealityCameraParent** instead of **MixedRealityCamera**.</span></span> <span data-ttu-id="d5e9d-449">此外需要添加**InputManager**并**DefaultCusor**。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-449">You also need to add **InputManager** and **DefaultCusor**.</span></span> <span data-ttu-id="d5e9d-450">由于**MixedRealityCameraParent**已包括**MotionControllers**并**边界**作为子组件，应删除现有**MotionControllers**并**环境**预设。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-450">Since **MixedRealityCameraParent** already includes **MotionControllers** and **Boundary** as child components, you should remove existing **MotionControllers** and **Environment** prefab.</span></span>

### <a name="instructions"></a><span data-ttu-id="d5e9d-451">说明</span><span class="sxs-lookup"><span data-stu-id="d5e9d-451">Instructions</span></span>

* <span data-ttu-id="d5e9d-452">在中**层次结构**面板中，删除**MixedRealityCamera**，**环境**和**MotionControllers**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-452">In the **Hierarchy** panel, delete **MixedRealityCamera**, **Environment** and **MotionControllers**</span></span>
* <span data-ttu-id="d5e9d-453">从**项目面板**，搜索并拖动到以下预设**层次结构**面板：</span><span class="sxs-lookup"><span data-stu-id="d5e9d-453">From the **Project panel**, search and drag the following prefabs into the **Hierarchy** panel:</span></span>
    * <span data-ttu-id="d5e9d-454">Assets/AppPrefabs/Input/Prefabs/**MixedRealityCameraParent**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-454">Assets/AppPrefabs/Input/Prefabs/**MixedRealityCameraParent**</span></span>
    * <span data-ttu-id="d5e9d-455">Assets/AppPrefabs/Input/Prefabs/**InputManager**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-455">Assets/AppPrefabs/Input/Prefabs/**InputManager**</span></span>
    * <span data-ttu-id="d5e9d-456">Assets/AppPrefabs/Input/Prefabs/Cursor/**DefaultCursor**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-456">Assets/AppPrefabs/Input/Prefabs/Cursor/**DefaultCursor**</span></span>

![混合的现实照相机父](images/mr213-cameraparent-300px.png)
* <span data-ttu-id="d5e9d-458">在中**层次结构**面板中，单击**输入管理器**</span><span class="sxs-lookup"><span data-stu-id="d5e9d-458">In the **Hierarchy** panel, click **Input Manager**</span></span>
* <span data-ttu-id="d5e9d-459">在中**Inspector**面板中，向下滚动到**简单单个指针选择器**部分</span><span class="sxs-lookup"><span data-stu-id="d5e9d-459">In the **Inspector** panel, scroll down to the **Simple Single Pointer Selector** section</span></span>
* <span data-ttu-id="d5e9d-460">从**层次结构**面板中，将**DefaultCursor**到**游标**字段</span><span class="sxs-lookup"><span data-stu-id="d5e9d-460">From the **Hierarchy** panel, drag **DefaultCursor** into **Cursor** field</span></span>

![分配 DefaultCursor](images/mr213-defaultcursor-500px.png)
* <span data-ttu-id="d5e9d-462">**保存**场景和单击**播放**按钮。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-462">**Save** the scene and click the **play** button.</span></span> <span data-ttu-id="d5e9d-463">你将能够使用摇杆左/右或 teleport 轮换一次。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-463">You will be able to use the thumbstick to rotate left/right or teleport.</span></span>

## <a name="the-end"></a><span data-ttu-id="d5e9d-464">结束</span><span class="sxs-lookup"><span data-stu-id="d5e9d-464">The end</span></span>

<span data-ttu-id="d5e9d-465">就是本教程结束 ！</span><span class="sxs-lookup"><span data-stu-id="d5e9d-465">And that's the end of this tutorial!</span></span> <span data-ttu-id="d5e9d-466">介绍了：</span><span class="sxs-lookup"><span data-stu-id="d5e9d-466">You learned:</span></span>
* <span data-ttu-id="d5e9d-467">如何使用 Unity 的游戏模式和运行时中的动作控制器模型。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-467">How to work with motion controller models in Unity's game mode and runtime.</span></span>
* <span data-ttu-id="d5e9d-468">如何使用不同类型的按钮事件和他们的应用程序。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-468">How to use different types of button events and their applications.</span></span>
* <span data-ttu-id="d5e9d-469">如何覆盖在控制器上的 UI 元素或完全自定义。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-469">How to overlay UI elements on top of the controller or fully customize it.</span></span>

<span data-ttu-id="d5e9d-470">现已准备好开始使用 motion 控制器创建自己的沉浸式体验 ！</span><span class="sxs-lookup"><span data-stu-id="d5e9d-470">You are now ready to start creating your own immersive experience with motion controllers!</span></span>

## <a name="completed-scenes"></a><span data-ttu-id="d5e9d-471">已完成的场景</span><span class="sxs-lookup"><span data-stu-id="d5e9d-471">Completed scenes</span></span>

* <span data-ttu-id="d5e9d-472">在 Unity 中的**项目**面板中单击**场景**文件夹。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-472">In Unity's **Project** panel click on the **Scenes** folder.</span></span>
* <span data-ttu-id="d5e9d-473">您会发现两个 Unity sceens **MixedReality213**并**MixedReality213Advanced**。</span><span class="sxs-lookup"><span data-stu-id="d5e9d-473">You will find two Unity sceens **MixedReality213** and **MixedReality213Advanced**.</span></span>
    * <span data-ttu-id="d5e9d-474">**MixedReality213**:已完成的场景中具有单个画笔</span><span class="sxs-lookup"><span data-stu-id="d5e9d-474">**MixedReality213**: Completed scene with single brush</span></span>
    * <span data-ttu-id="d5e9d-475">**MixedReality213Advanced**:使用选择按钮的按量示例的多个画笔的已完成的场景</span><span class="sxs-lookup"><span data-stu-id="d5e9d-475">**MixedReality213Advanced**: Completed scene with multiple brush with select button's press amount example</span></span>

## <a name="see-also"></a><span data-ttu-id="d5e9d-476">请参阅</span><span class="sxs-lookup"><span data-stu-id="d5e9d-476">See also</span></span>

* [<span data-ttu-id="d5e9d-477">MR 输入 213 项目文件</span><span class="sxs-lookup"><span data-stu-id="d5e9d-477">MR Input 213 project files</span></span>](https://github.com/Microsoft/MixedReality213)
* [<span data-ttu-id="d5e9d-478">混合的现实工具包-运动控制器测试场景</span><span class="sxs-lookup"><span data-stu-id="d5e9d-478">Mixed Reality Toolkit - Motion Controller Test scene</span></span>](https://github.com/Microsoft/MixedRealityToolkit-Unity/tree/htk_release/Assets/HoloToolkit-Examples/Input/Scenes)
* [<span data-ttu-id="d5e9d-479">混合的现实工具包-随取即行机制</span><span class="sxs-lookup"><span data-stu-id="d5e9d-479">Mixed Reality Toolkit - Grab Mechanics</span></span>](https://github.com/Microsoft/MixedRealityToolkit-Unity/tree/htk_release/Assets/HoloToolkit-Examples/MotionControllers-GrabMechanics)
* [<span data-ttu-id="d5e9d-480">运动控制器开发指导原则</span><span class="sxs-lookup"><span data-stu-id="d5e9d-480">Motion controller development guidelines</span></span>](motion-controllers.md)
