...
## <a name="before-you-start"></a><span data-ttu-id="c31d3-151">开始之前</span><span class="sxs-lookup"><span data-stu-id="c31d3-151">Before you start</span></span>
...
4.  <span data-ttu-id="c31d3-164">一旦你单击**创建**:</span><span class="sxs-lookup"><span data-stu-id="c31d3-164">Once you have clicked on **Create**:</span></span>
    ...
    2.  <span data-ttu-id="c31d3-166">有关*部署模型*，选择**资源管理器**。</span><span class="sxs-lookup"><span data-stu-id="c31d3-166">For *Deployment model*, select **Resource manager**.</span></span>
...
1.  <span data-ttu-id="c31d3-250">打开 Unity，然后单击**新建**。</span><span class="sxs-lookup"><span data-stu-id="c31d3-250">Open Unity and click **New**.</span></span>
...
5.  <span data-ttu-id="c31d3-265">转到**文件 > 生成设置**并确保选中：</span><span class="sxs-lookup"><span data-stu-id="c31d3-265">Go to **File > Build Settings** and make sure that:</span></span>
    ...
    5. <span data-ttu-id="c31d3-271">**生成并运行**设置为**本地计算机**</span><span class="sxs-lookup"><span data-stu-id="c31d3-271">**Build and Run** is set to **Local Machine**</span></span>
...
8.  <span data-ttu-id="c31d3-284">在此面板中，需要验证几个设置：</span><span class="sxs-lookup"><span data-stu-id="c31d3-284">In this panel, a few settings need to be verified:</span></span>
    ...
    2.  <span data-ttu-id="c31d3-289">内**发布设置**选项卡上，在**功能**，检查：</span><span class="sxs-lookup"><span data-stu-id="c31d3-289">Within the **Publishing Settings** tab, under **Capabilities**, check:</span></span>
        ...
        -  <span data-ttu-id="c31d3-290">**InternetClient**</span><span class="sxs-lookup"><span data-stu-id="c31d3-290">**InternetClient**</span></span>
...
3.  <span data-ttu-id="c31d3-306">若要执行此操作，请选择**齿轮**旁的摄像机*转换*组件，然后选择**重置**。</span><span class="sxs-lookup"><span data-stu-id="c31d3-306">To do this, select the **Gear** icon next to the Camera's *Transform* component, and select **Reset**.</span></span>
...
2.  <span data-ttu-id="c31d3-334">与**平面**对象选择，更改中的以下参数*检查器面板*:</span><span class="sxs-lookup"><span data-stu-id="c31d3-334">With the **Plane** object selected, change the following parameters in the *Inspector Panel*:</span></span>
    ...
    | <span data-ttu-id="c31d3-346">10</span><span class="sxs-lookup"><span data-stu-id="c31d3-346">10</span></span>    | <span data-ttu-id="c31d3-347">1</span><span class="sxs-lookup"><span data-stu-id="c31d3-347">1</span></span>                 | <span data-ttu-id="c31d3-348">10</span><span class="sxs-lookup"><span data-stu-id="c31d3-348">10</span></span>    |
...
5.  <span data-ttu-id="c31d3-371">选择新的对象，并将其重命名**ShapeSpawnPoint**。</span><span class="sxs-lookup"><span data-stu-id="c31d3-371">Select the new object and rename it **ShapeSpawnPoint**.</span></span>
...
4.  <span data-ttu-id="c31d3-421">转到*存储*下的文件夹*插件*，在项目视图中，选择以下插件*仅*:</span><span class="sxs-lookup"><span data-stu-id="c31d3-421">Go to the *Storage* folder under *Plugins*, in the Project view, and select the following plugins *only*:</span></span>
...
<span data-ttu-id="c31d3-440">要创建的第一个类是*azure 服务*类。</span><span class="sxs-lookup"><span data-stu-id="c31d3-440">The first class you are going to create is the *AzureServices* class.</span></span>
...
11. <span data-ttu-id="c31d3-463">单击并拖动*azure 服务*从脚本文件夹中的 Main Camera 对象的类*层次结构面板*。</span><span class="sxs-lookup"><span data-stu-id="c31d3-463">Click and drag the *AzureServices* class from the Scripts folder to the Main Camera object in the *Hierarchy Panel*.</span></span>
...
5.  <span data-ttu-id="c31d3-477">添加到如下所示的变量*ShapeFactory*类，并替换*start （)* 并*Awake()* 与下面的函数：</span><span class="sxs-lookup"><span data-stu-id="c31d3-477">Add the variables shown below to the *ShapeFactory* class, and replace the *Start()* and *Awake()* functions with those below:</span></span>
...
<span data-ttu-id="c31d3-487">若要创建所需的最后一个脚本是*注视*类。</span><span class="sxs-lookup"><span data-stu-id="c31d3-487">The last script you need to create is the *Gaze* class.</span></span>
...
> [!IMPORTANT]
> <span data-ttu-id="c31d3-497">其中一些变量将能够在中进行编辑*编辑器*。</span><span class="sxs-lookup"><span data-stu-id="c31d3-497">Some of these variables will be able to be edited in the *Editor*.</span></span>

6.  <span data-ttu-id="c31d3-498">为代码*Awake()* 并*start （)* 方法现在需要添加。</span><span class="sxs-lookup"><span data-stu-id="c31d3-498">Code for the *Awake()* and *Start()* methods now needs to be added.</span></span>
...
7.  <span data-ttu-id="c31d3-499">添加以下代码，将创建游标对象处开始，连同*update （)* 方法，将运行 Raycast 方法，以及在其中切换 GazeEnabled 布尔值：</span><span class="sxs-lookup"><span data-stu-id="c31d3-499">Add the following code, which will create a cursor object at start, along with the *Update()* method, which will run the Raycast method, along with being where the GazeEnabled boolean is toggled:</span></span>
...
8. <span data-ttu-id="c31d3-500">接下来，添加*UpdateRaycast()* 方法，它将项目 Raycast 并检测命中的目标。</span><span class="sxs-lookup"><span data-stu-id="c31d3-500">Next add the *UpdateRaycast()* method, which will project a Raycast and detect the hit target.</span></span>
...
9. <span data-ttu-id="c31d3-501">最后，将添加*ResetFocusedObject()* 方法，它将切换 GazeButton 对象当前颜色，指示它是否创建一个新的形状。</span><span class="sxs-lookup"><span data-stu-id="c31d3-501">Lastly, add the *ResetFocusedObject()* method, which will toggle the GazeButton objects current color, indicating whether it is creating a new shape or not.</span></span>
...
10.  <span data-ttu-id="c31d3-502">返回到 Unity 之前，Visual Studio 中保存所做的更改。</span><span class="sxs-lookup"><span data-stu-id="c31d3-502">Save your changes in Visual Studio before returning to Unity.</span></span>

11.  <span data-ttu-id="c31d3-503">单击并拖动*注视*到脚本文件夹中的类**Main Camera**对象中*层次结构面板*。</span><span class="sxs-lookup"><span data-stu-id="c31d3-503">Click and drag the *Gaze* class from the Scripts folder to the **Main Camera** object in the *Hierarchy Panel*.</span></span>

## <a name="chapter-10---completing-the-azureservices-class"></a><span data-ttu-id="c31d3-504">第 10 章-完成 azure 服务类</span><span class="sxs-lookup"><span data-stu-id="c31d3-504">Chapter 10 - Completing the AzureServices class</span></span>

<span data-ttu-id="c31d3-505">与其他脚本中的位置，现在就可以*完整* *azure 服务*类。</span><span class="sxs-lookup"><span data-stu-id="c31d3-505">With the other scripts in place, it is now possible to *complete* the *AzureServices* class.</span></span> <span data-ttu-id="c31d3-506">将通过实现此目的：</span><span class="sxs-lookup"><span data-stu-id="c31d3-506">This will be achieved through:</span></span>

1.  <span data-ttu-id="c31d3-507">添加名为的新方法*CreateCloudIdentityAsync()*，若要设置用于与 Azure 进行通信所需的身份验证变量。</span><span class="sxs-lookup"><span data-stu-id="c31d3-507">Adding a new method named *CreateCloudIdentityAsync()*, to set up the authentication variables needed for communicating with Azure.</span></span>

    > <span data-ttu-id="c31d3-508">此方法还将检查存在包含形状列表的以前存储文件。</span><span class="sxs-lookup"><span data-stu-id="c31d3-508">This method will also check for the existence of a previously stored File containing the Shape List.</span></span>
    >
    > <span data-ttu-id="c31d3-509">**如果找到该文件**，它将禁用的用户*注视*，并触发形状创建过程中，根据形状的模式，如存储在**Azure 存储文件**。</span><span class="sxs-lookup"><span data-stu-id="c31d3-509">**If the file is found**, it will disable the user *Gaze*, and trigger Shape creation, according to the pattern of shapes, as stored in the **Azure Storage file**.</span></span> <span data-ttu-id="c31d3-510">用户可以看到这一点，作为**文本 Mesh**将提供显示存储或 New，具体取决于形状原点。</span><span class="sxs-lookup"><span data-stu-id="c31d3-510">The user can see this, as the **Text Mesh** will provide display 'Storage' or 'New', depending on the shapes origin.</span></span>
    >
    > <span data-ttu-id="c31d3-511">**如果未找到文件**，它可实现*注视*，使用户可以在查看时创建的形状**GazeButton**场景中的对象。</span><span class="sxs-lookup"><span data-stu-id="c31d3-511">**If no file is found**, it will enable the *Gaze*, enabling the user to create shapes when looking at the **GazeButton** object in the scene.</span></span>
...
2.  <span data-ttu-id="c31d3-512">下一步的代码片段是从*start （)* 方法; 其中一个调用将对*CreateCloudIdentityAsync()* 方法。</span><span class="sxs-lookup"><span data-stu-id="c31d3-512">The next code snippet is from within the *Start()* method; wherein a call will be made to the *CreateCloudIdentityAsync()* method.</span></span> <span data-ttu-id="c31d3-513">可随意复制您的当前*start （)* 方法中，使用如下：</span><span class="sxs-lookup"><span data-stu-id="c31d3-513">Feel free to copy over your current *Start()* method, with the below:</span></span>
...
3.  <span data-ttu-id="c31d3-514">方法的代码中填写*CallAzureFunctionForNextShape()*。</span><span class="sxs-lookup"><span data-stu-id="c31d3-514">Fill in the code for the method *CallAzureFunctionForNextShape()*.</span></span> <span data-ttu-id="c31d3-515">您将使用之前创建*Azure Function App*来请求一个形状的索引。</span><span class="sxs-lookup"><span data-stu-id="c31d3-515">You will use the previously created *Azure Function App* to request a shape index.</span></span> <span data-ttu-id="c31d3-516">此方法后收到新形状时，会将发送到形状*ShapeFactory*类，以在场景中创建新的形状。</span><span class="sxs-lookup"><span data-stu-id="c31d3-516">Once the new shape is received, this method will send the shape to the *ShapeFactory* class to create the new shape in the scene.</span></span> <span data-ttu-id="c31d3-517">使用下面的代码来完成的正文*CallAzureFunctionForNextShape()*。</span><span class="sxs-lookup"><span data-stu-id="c31d3-517">Use the code below to complete the body of *CallAzureFunctionForNextShape()*.</span></span>
...
4.  <span data-ttu-id="c31d3-518">添加一个方法来创建字符串，通过串联整数存储在形状历史记录列表中，并保存在你*Azure 存储文件*。</span><span class="sxs-lookup"><span data-stu-id="c31d3-518">Add a method to create a string, by concatenating the integers stored in the shape history list, and saving it in your *Azure Storage File*.</span></span>
...
5.  <span data-ttu-id="c31d3-519">添加一个方法以便检索存储在文件中的文本你*Azure 存储文件*并*反序列化*到列表。</span><span class="sxs-lookup"><span data-stu-id="c31d3-519">Add a method to retrieve the text stored in the file located in your *Azure Storage File* and *deserialize* it into a list.</span></span>

6.  <span data-ttu-id="c31d3-520">完成此过程后，该方法将重新启用的视线移动，以便用户可以向场景添加更多的形状。</span><span class="sxs-lookup"><span data-stu-id="c31d3-520">Once this process is completed, the method re-enables the gaze so that the user can add more shapes to the scene.</span></span>
...
7.  <span data-ttu-id="c31d3-521">返回到 Unity 之前，Visual Studio 中保存所做的更改。</span><span class="sxs-lookup"><span data-stu-id="c31d3-521">Save your changes in Visual Studio before returning to Unity.</span></span>

## <a name="chapter-11---build-the-uwp-solution"></a><span data-ttu-id="c31d3-522">第 11 章 — 生成 UWP 解决方案</span><span class="sxs-lookup"><span data-stu-id="c31d3-522">Chapter 11 - Build the UWP Solution</span></span>

<span data-ttu-id="c31d3-523">若要开始生成过程：</span><span class="sxs-lookup"><span data-stu-id="c31d3-523">To begin the Build process:</span></span>

1.  <span data-ttu-id="c31d3-524">转到**文件 > 生成设置**。</span><span class="sxs-lookup"><span data-stu-id="c31d3-524">Go to **File > Build Settings**.</span></span>

    ![生成应用](images/AzureLabs-Lab5-54.png)

2.  <span data-ttu-id="c31d3-526">单击“生成” 。</span><span class="sxs-lookup"><span data-stu-id="c31d3-526">Click **Build**.</span></span> <span data-ttu-id="c31d3-527">将启动 unity*文件资源管理器*窗口中，需要创建，然后选择要生成该应用程序的文件夹。</span><span class="sxs-lookup"><span data-stu-id="c31d3-527">Unity will launch a *File Explorer* window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="c31d3-528">现在，创建该文件夹并将其命名*应用*。</span><span class="sxs-lookup"><span data-stu-id="c31d3-528">Create that folder now, and name it *App*.</span></span> <span data-ttu-id="c31d3-529">然后使用*应用程序*文件夹选择，按**选择文件夹**。</span><span class="sxs-lookup"><span data-stu-id="c31d3-529">Then with the *App* folder selected, press **Select Folder**.</span></span>

3.  <span data-ttu-id="c31d3-530">Unity 将开始构建您的项目*应用*文件夹。</span><span class="sxs-lookup"><span data-stu-id="c31d3-530">Unity will begin building your project to the *App* folder.</span></span>

4.  <span data-ttu-id="c31d3-531">一次 Unity 已完成的生成 （它可能需要一些时间），它将打开*文件资源管理器*窗口在生成的位置 （检查任务栏中，因为它可能不总是会显示您的窗口上方，但会通知你添加了一个新窗口）。</span><span class="sxs-lookup"><span data-stu-id="c31d3-531">Once Unity has finished building (it might take some time), it will open a *File Explorer* window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

## <a name="chapter-12---deploying-your-application"></a><span data-ttu-id="c31d3-532">第 12 章 — 部署应用程序</span><span class="sxs-lookup"><span data-stu-id="c31d3-532">Chapter 12 - Deploying your application</span></span>

<span data-ttu-id="c31d3-533">若要部署你的应用程序：</span><span class="sxs-lookup"><span data-stu-id="c31d3-533">To deploy your application:</span></span>

1.  <span data-ttu-id="c31d3-534">导航到*应用程序*文件夹中创建[最后一章](#chapter-11---build-the-uwp-solution)。</span><span class="sxs-lookup"><span data-stu-id="c31d3-534">Navigate to the *App* folder which was created in the [last Chapter](#chapter-11---build-the-uwp-solution).</span></span> <span data-ttu-id="c31d3-535">您将查看你的应用名称，使用.sln 扩展，则您应该双击了文件，因此，若要中打开它*Visual Studio*。</span><span class="sxs-lookup"><span data-stu-id="c31d3-535">You will see a file with your apps name, with the '.sln' extension, which you should double-click, so to open it within *Visual Studio*.</span></span>

2.  <span data-ttu-id="c31d3-536">在中**解决方案平台**，选择**x86，本地计算机**。</span><span class="sxs-lookup"><span data-stu-id="c31d3-536">In the **Solution Platform**, select **x86, Local Machine**.</span></span>

3.  <span data-ttu-id="c31d3-537">在中**解决方案配置**选择**调试**。</span><span class="sxs-lookup"><span data-stu-id="c31d3-537">In the **Solution Configuration** select **Debug**.</span></span>

    > <span data-ttu-id="c31d3-538">对于 Microsoft HoloLens，您可能会发现它更轻松地将其设置为*远程计算机*，因此您不已接入到你的计算机。</span><span class="sxs-lookup"><span data-stu-id="c31d3-538">For the Microsoft HoloLens, you may find it easier to set this to *Remote Machine*, so that you are not tethered to your computer.</span></span> <span data-ttu-id="c31d3-539">不过，需要还执行以下操作：</span><span class="sxs-lookup"><span data-stu-id="c31d3-539">Though, you will need to also do the following:</span></span>
    > - <span data-ttu-id="c31d3-540">知道**IP 地址**的你 HoloLens，在中找到*设置 > 网络和 Internet > Wi-fi > 高级选项*; IPv4 是应使用的地址。</span><span class="sxs-lookup"><span data-stu-id="c31d3-540">Know the **IP Address** of your HoloLens, which can be found within the *Settings > Network & Internet > Wi-Fi > Advanced Options*; the IPv4 is the address you should use.</span></span> 
    > - <span data-ttu-id="c31d3-541">请确保**开发人员模式**是**上**; 中找到*设置 > 更新和安全 > 面向开发人员*。</span><span class="sxs-lookup"><span data-stu-id="c31d3-541">Ensure **Developer Mode** is **On**; found in *Settings > Update & Security > For developers*.</span></span>

    ![部署解决方案](images/AzureLabs-Lab5-55.png)

4.  <span data-ttu-id="c31d3-543">转到**构建**菜单，并单击**部署解决方案**旁加载到计算机中，应用程序。</span><span class="sxs-lookup"><span data-stu-id="c31d3-543">Go to the **Build** menu and click on **Deploy Solution** to sideload the application to your machine.</span></span>

5.  <span data-ttu-id="c31d3-544">你的应用现在应已安装的应用，准备好进行启动和测试的列表中显示 ！</span><span class="sxs-lookup"><span data-stu-id="c31d3-544">Your App should now appear in the list of installed apps, ready to be launched and tested!</span></span>

## <a name="your-finished-azure-functions-and-storage-application"></a><span data-ttu-id="c31d3-545">你已完成的 Azure 函数和存储应用程序</span><span class="sxs-lookup"><span data-stu-id="c31d3-545">Your finished Azure Functions and Storage Application</span></span>

<span data-ttu-id="c31d3-546">祝贺你，构建利用 Azure Functions 和 Azure 存储服务的混合的现实应用。</span><span class="sxs-lookup"><span data-stu-id="c31d3-546">Congratulations, you built a mixed reality app that leverages both the Azure Functions and Azure Storage services.</span></span> <span data-ttu-id="c31d3-547">您的应用程序将能够利用存储的数据，并提供一个操作基于该数据。</span><span class="sxs-lookup"><span data-stu-id="c31d3-547">Your app will be able to draw on stored data, and provide an action based on that data.</span></span>

![最终产品的最终](images/AzureLabs-Lab5-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="c31d3-549">Bonus 练习</span><span class="sxs-lookup"><span data-stu-id="c31d3-549">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="c31d3-550">练习 1</span><span class="sxs-lookup"><span data-stu-id="c31d3-550">Exercise 1</span></span>

<span data-ttu-id="c31d3-551">创建第二个生成点和记录的生成的点从创建了一个对象。</span><span class="sxs-lookup"><span data-stu-id="c31d3-551">Create a second spawn point and record which spawn point an object was created from.</span></span> <span data-ttu-id="c31d3-552">当数据文件加载时，重播正在从最初创建它们的位置生成的形状。</span><span class="sxs-lookup"><span data-stu-id="c31d3-552">When you load the data file, replay the shapes being spawned from the location they originally were created.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="c31d3-553">练习 2</span><span class="sxs-lookup"><span data-stu-id="c31d3-553">Exercise 2</span></span>

<span data-ttu-id="c31d3-554">创建应用程序中，而不是无需重新打开它每次重新启动的方法。</span><span class="sxs-lookup"><span data-stu-id="c31d3-554">Create a way to restart the app, rather than having to re-open it each time.</span></span> <span data-ttu-id="c31d3-555">**正在加载场景**是启动操作的好地方。</span><span class="sxs-lookup"><span data-stu-id="c31d3-555">**Loading Scenes** is a good spot to start.</span></span> <span data-ttu-id="c31d3-556">执行这些操作之后, 创建一种方法来清除中的存储的列表*Azure 存储*，以便它可以轻松地重置从您的应用程序。</span><span class="sxs-lookup"><span data-stu-id="c31d3-556">After doing that, create a way to clear the stored list in *Azure Storage*, so that it can be easily reset from your app.</span></span> 
