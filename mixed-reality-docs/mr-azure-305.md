...
6.  接下来，你将创建一个**3D 文本**对象，以提供有关 Azure 服务状态的反馈。
    ...
    再次右键单击 "层次结构" 面板中的**GazeButton** ，并将**3D 对象** > **3d 文本**对象添加为*子*对象。
...
2.  在弹出的 "**导入 Unity 包**" 框中，可以选择 > **存储**的**插件**下的所有内容。 取消选中所有其他内容，因为本课程不需要这样做。
...
1.  右键单击 "*资产*" 文件夹，在 "项目" 面板中，**创建** > **文件夹**。 命名文件夹**脚本**。
...
3.  右键单击文件夹内部，**创建** >  **C#脚本**。 调用脚本*azure 服务*。
...
2.  右键单击文件夹内部，**创建** >  **C#脚本**。 调用脚本*ShapeFactory*。
...
3.  在**解决方案配置**中，选择 "**调试**"。
    ...
    > - 确保**开发人员模式**已**打开**;在 "**设置**" 中找到 > 更新**开发人员** **& 安全** > 。
